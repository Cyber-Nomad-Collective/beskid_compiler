...
trap 'rm -rf "$fixture_root" "$deprecated_root"' EXIT
...
if BESKID_HIR_FREE_SCAN_ROOT="$fixture_root" "$workspace/scripts/verify-hir-free-abi-v5.sh" --static-only >"$output" 2>&1; then
...
if BESKID_HIR_FREE_SCAN_ROOT="$deprecated_root" "$workspace/scripts/verify-hir-free-abi-v5.sh" --static-only >"$deprecated_output" 2>&1; then
