...
fn run_one(action: &TemplatePostAction, ctx: &PostActionContext) -> TemplateResult<()> {
    match action.action_id.as_str() {
        ...
        other => {
            if ctx.strict {
                return Err(TemplateError::Internal(format!("unknown post-action `{other}`")));
            }
...
fn beskid_command(ctx: &PostActionContext) -> PathBuf {
    ctx.beskid_exe.clone().unwrap_or_else(|| std::env::current_exe().unwrap_or_else(|_| PathBuf::from("beskid")))
}
...
fn run_beskid_lock(ctx: &PostActionContext) -> TemplateResult<()> {
    ...
    if project.is_none() {
        ...
        if workspace.is_none() {
            eprintln!("warning: skipping beskidLock — no `.bproj` or `.bws` manifest at {}", ctx.lock_root.display());
            return Ok(());
        }
...
fn discover_project_manifest(root: &Path) -> Option<PathBuf> {
    beskid_analysis::projects::discover_project_manifest_in_dir(root).ok().flatten()
}

fn discover_workspace_manifest(root: &Path) -> Option<PathBuf> {
    beskid_analysis::projects::discover_workspace_manifest_in_dir(root).ok().flatten()
}
...
fn run_command(action: &TemplatePostAction, ctx: &PostActionContext) -> TemplateResult<()> {
    ...
    let status =
        Command::new("sh").arg("-c").arg(command).current_dir(&ctx.output_root).status().map_err(TemplateError::Io)?;
    if !status.success() {
        return Err(TemplateError::Internal(format!("runCommand failed with status {status}")));
    }
    Ok(())
}

fn open_readme(action: &TemplatePostAction, ctx: &PostActionContext) -> TemplateResult<()> {
    let path = action.args.get("path").and_then(|v| v.as_str()).unwrap_or("README.md");
