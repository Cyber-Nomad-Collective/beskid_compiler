...
use beskid_isle::{
    ArrayLayout, AstNodeKey, FunctionEmissionError, FunctionEmitter, LiteralKind,
...
impl NodeFacts for ArrayFacts {
    ...
    fn literal_kind(&self, key: AstNodeKey) -> Option<LiteralKind> {
        match self.root {
            Root::IndexRead => self.nodes[2..]
                .contains(&key)
                .then_some(LiteralKind::Integer),
            Root::IndexAssign => self.nodes[3..]
                .contains(&key)
                .then_some(LiteralKind::Integer),
        }
    }
    ...
    fn child(&self, key: AstNodeKey, index: u8) -> Option<AstNodeKey> {
        match self.root {
            Root::IndexRead if key == self.nodes[0] => [self.nodes[1], self.nodes[2]]
                .get(usize::from(index))
                .copied(),
            Root::IndexAssign if key == self.nodes[0] => [self.nodes[1], self.nodes[3]]
                .get(usize::from(index))
                .copied(),
            Root::IndexAssign if key == self.nodes[1] => [self.nodes[2], self.nodes[4]]
                .get(usize::from(index))
                .copied(),
            _ => None,
        }
    ...
    fn integer_literal(&self, key: AstNodeKey) -> Option<i64> {
        match self.root {
            ...
            Root::IndexRead => self.nodes[3..]
                .iter()
                .position(|candidate| *candidate == key)
                .map(|index| (index as i64 + 1) * 10),
    ...
    fn scalar_type(&self, key: AstNodeKey) -> Option<cranelift_codegen::ir::Type> {
        match self.root {
            ...
            Root::IndexRead if key == self.nodes[0] || self.nodes[2..].contains(&key) => {
                Some(types::I32)
            }
...
fn array_literal_and_index_emit_checked_stock_clif_and_execute() {
    ...
    let function_id = module
        .declare_function("array_index", Linkage::Local, &signature)
        .expect("declare");
...
fn array_index_assignment_emits_checked_stock_clif_store_and_executes() {
    ...
    let mut module = JITModule::new(JITBuilder::with_isa(isa, default_libcall_names()));
    let function_id = module
        .declare_function("array_index_assign", Linkage::Local, &signature)
        .expect("declare");
    let mut context = module.make_context();
    context.func = function;
    module
