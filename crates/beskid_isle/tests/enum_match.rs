...
struct MultiFieldEnumFacts {
    nodes: [AstNodeKey; 3],
    pointer_type: Type,
}

impl NodeFacts for MultiFieldEnumFacts {
    fn node_kind(&self, key: AstNodeKey) -> Option<NodeKind> {
        if key == self.nodes[0] {
            Some(NodeKind::EnumLiteralExpression)
    ...
    fn literal_kind(&self, key: AstNodeKey) -> Option<LiteralKind> {
        self.nodes[1..].contains(&key).then_some(LiteralKind::Integer)
    }

    fn integer_literal(&self, key: AstNodeKey) -> Option<i64> {
        (key == self.nodes[1]).then_some(11).or_else(|| (key == self.nodes[2]).then_some(22))
    }
    ...
    fn enum_layout(&self, key: AstNodeKey) -> Option<EnumLayout> {
        (key == self.nodes[0]).then(|| {
            EnumLayout::new(
                ...
                2,
                FieldLayout::new(types::I32, 0),
                vec![EnumVariantLayout::with_fields(
                    0,
                    vec![FieldLayout::new(types::I32, 4), FieldLayout::new(types::I32, 8)],
                )],
            )
        })
    }
    ...
    fn enum_variant_index(&self, key: AstNodeKey) -> Option<u32> {
        (key == self.nodes[0]).then_some(0)
    }

    fn enum_payloads(&self, key: AstNodeKey) -> Option<Vec<AstNodeKey>> {
        (key == self.nodes[0]).then(|| self.nodes[1..].to_vec())
    }

    fn managed_struct_allocation(&self, key: AstNodeKey) -> Option<ManagedStructAllocation> {
        (key == self.nodes[0]).then(|| ManagedStructAllocation {
            allocation_request_symbol: "__test_multi_field_enum_allocation_request".into(),
        })
    }
}

#[test]
fn multi_field_enum_literal_stores_every_source_ordered_payload() {
    let isa = cranelift_codegen::isa::lookup(Triple::host())
        .expect("host ISA")
        .finish(settings::Flags::new(settings::builder()))
        .expect("host flags");
    let db = BeskidDatabase::default();
    let unit = SourceUnitId::new(&db, PathBuf::from("/tmp/MultiFieldEnum.bd"));
    let facts = MultiFieldEnumFacts {
        nodes: std::array::from_fn(|index| AstNodeKey {
            unit,
            generation: SyntaxGenerationId(20),
    ...
    let emitter = FunctionEmitter::new(isa.as_ref());
    let function = emitter
        .emit_expression(UserFuncName::user(0, 33), emitter.signature([], [isa.pointer_type()]), &facts, facts.nodes[0])
        .expect("multi-field enum constructor lowers");
    let clif = function.display().to_string();
    assert!(clif.contains("iconst.i32 11"), "{clif}");
    assert!(clif.contains("iconst.i32 22"), "{clif}");
    assert!(clif.contains("store v3, v1+4"), "{clif}");
    assert!(clif.contains("store v4, v1+8"), "{clif}");
}
