...
fn map_path_type(
            ...
            if let Some(h) = helpers {
                if let Some(vel) = vec_element_type(p) {
                    ...
                    if let Some(el) = list_element_rust_name(vel) {
                        if let Some(list_path) = h.list_by_element.get(&el) {
                ...
                if let Some(nm) = option_payload_rust_name(p) {
                    if let Some(opt_path) = h.optional_by_inner.get(&nm) {
                        return TypeMirror {
                            beskid_ty: opt_path.clone(),
                            stub_note: None,
                        };
    ...
    if let ("Vec", PathArguments::AngleBracketed(ab)) = (ident.as_str(), args) {
        if let Some(GenericArgument::Type(inner)) = ab.args.first() {
            ...
            if let Some(h) = helpers {
                if let Some(el) = list_element_rust_name(pi) {
                    if let Some(p) = h.list_by_element.get(&el) {
                        return TypeMirror {
                            beskid_ty: p.clone(),
                            stub_note: None,
                        };
...
pub fn emit_syntax_sdk(
    ...
    let mut barrel_names: Vec<String> = helpers.helper_emit_order.clone();
    let mut extra: Vec<String> = decls
        .keys()
        .filter(|k| !helpers.helper_emit_order.iter().any(|h| h == *k))
        .cloned()
        .collect();
    extra.sort();
...
"#
    ...
    let mut expected_bd: BTreeSet<String> = helpers.helper_emit_order.iter().cloned().collect();
    expected_bd.extend(decls.keys().cloned());
