...
mod tests {
    ...
    #[test]
    fn heap_reports_idle_phase_after_forced_collection() {
        let ctx = GcContext::with_options(GcOptions::OFF);
        let _root = ctx.allocate(42);

        ctx.heap().force_collect();

        let stats = ctx.heap().stats();
        assert_eq!(stats.phase, GcPhase::Idle);
        assert_eq!(ctx.heap().gc_phase(), GcPhase::Idle);
        assert_eq!(stats.bytes_allocated, ctx.heap().bytes_allocated());
        assert_eq!(stats.object_count, ctx.heap().allocation_count());
    }
}
