...
pub enum ResolveError {
    ...
    DuplicateSymbol { symbol: String, span: SpanInfo, previous: SpanInfo },
    ...
    UnknownValueInModule { module_path: String, name: String, span: SpanInfo },
    UnknownTypeInModule { module_path: String, name: String, span: SpanInfo },
    InvalidConformanceTarget { name: String, span: SpanInfo },
    PrivateItemInModule { module_path: String, name: String, span: SpanInfo },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResolveError::DuplicateItem { name, span, previous } => {
                write!(f, "duplicate item `{name}` at {} (previous at {})", span_loc(*span), span_loc(*previous))
            }
            ResolveError::DuplicateLocal { name, span, previous } => {
                write!(f, "duplicate local `{name}` at {} (previous at {})", span_loc(*span), span_loc(*previous))
            }
            ResolveError::DuplicateSymbol { symbol, span, previous } => {
                write!(f, "duplicate symbol `{symbol}` at {} (previous at {})", span_loc(*span), span_loc(*previous))
            }
            ...
            ResolveError::UnknownValueInModule { module_path, name, span } => {
                write!(f, "unknown value `{name}` in module `{module_path}` at {}", span_loc(*span))
            }
            ResolveError::UnknownTypeInModule { module_path, name, span } => {
                write!(f, "unknown type `{name}` in module `{module_path}` at {}", span_loc(*span))
            }
            ResolveError::InvalidConformanceTarget { name, span } => {
                write!(f, "invalid conformance target `{name}` at {}", span_loc(*span))
            }
            ResolveError::PrivateItemInModule { module_path, name, span } => {
                write!(f, "private item `{name}` in module `{module_path}` at {}", span_loc(*span))
            }
        }
    }
}
