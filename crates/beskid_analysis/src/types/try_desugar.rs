...
pub fn try_desugar_target_for_operand(
    ...
    let surface = UnitTypeSurface::default();
    let mut checker = TypeChecker::new(resolution, &surface);
    for program in programs {
        checker.seed_program_enums(program);
    }
    let target_type = checker.infer_expression_type(operand)?;
    let item_id = checker.item_for_type_id(target_type)?;
    let type_name = resolution
        .items
        .iter()
        .find(|info| info.id == item_id)
        .map(|info| info.name.clone())?;
    let ok_variant = checker.variant_display_name(item_id, "Ok")?;
...
fn is_array_iterable(
    ...
    let Some(target_type) = checker.infer_expression_type(iterable) else {
        return false;
    };
    matches!(checker.type_table.get(target_type), Some(TypeInfo::Array(_)))
}
