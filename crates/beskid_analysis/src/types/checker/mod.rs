...
impl<'a> TypeChecker<'a> {
    pub fn new(resolution: &'a Resolution, surface: &UnitTypeSurface) -> Self {
        let mut c = Self {
            resolution,
            type_table: surface.types.clone(),
            ...
            enum_variants_ordered: surface.enum_variants_ordered.clone(),
    ...
    pub(super) fn infer_local_type_from_expression(
        ...
        self.contextual_expected_type = None;
        let actual = self.type_expression(expression)?;
        self.contextual_expected_type = prev;
        self.record_node_type(expression.id, actual);
        self.constraints.equal(var, actual, expression.span);
        self.insert_local_type(local_span, actual);
        Some(actual)
    }
    fn seed_builtin_signatures(&mut self) {
        for (item_id, index) in &self.resolution.builtin_items {
            ...
            for p in spec.params {
                if let Some(t) = self.builtin_surface_type_id(spec, *p, false) {
                    params.push(t);
                }
            }
