...
impl fmt::Display for TypeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            TypeError::TypeMismatch { span, expected, actual } => write!(
                f,
            ...
            TypeError::MatchArmTypeMismatch { span, expected, actual } => write!(
                f,
            ...
            TypeError::CallArityMismatch { span, expected, actual } => {
                write!(f, "call arity mismatch at {}: expected {expected} arguments, got {actual}", at(*span))
            }
            TypeError::CallArgumentMismatch { span, expected, actual } => write!(
                f,
            ...
            TypeError::EnumConstructorMismatch { span, expected, actual } => write!(
                f,
            ...
            TypeError::JoinWouldDeadlock { span } => {
                write!(f, "join would deadlock: child fiber cannot join an ancestor handle at {}", at(*span))
            }
            TypeError::StackReferenceEscapesSpawn { span } => {
                write!(f, "stack reference would escape across spawn boundary at {}", at(*span))
            }
            TypeError::ReturnTypeMismatch { span, expected, actual } => match actual {
                Some(a) => write!(
                    f,
            ...
            TypeError::IterableNextArityMismatch { span, expected, actual } => {
                write!(f, "iterable `next` arity mismatch at {}: expected {expected} bindings, got {actual}", at(*span))
            }
            TypeError::IterableNextReturnNotOption { span } => {
                write!(f, "iterable `next` must return an option-like type at {}", at(*span))
            }
...
pub enum CallLoweringKind {
    MethodDispatch { method_item_id: ItemId, receiver_source: MethodReceiverSource, receiver_type: TypeId },
    ContractDispatch { contract_item_id: ItemId, receiver_source: MethodReceiverSource, receiver_type: TypeId },
    ItemCall { item_id: ItemId },
    EventInvoke { receiver_source: MethodReceiverSource, receiver_type: TypeId },
    CallableValueCall,
}
...
impl TypeResult {
    ...
    pub fn cast_intents_for_span(&self, span: SpanInfo) -> impl Iterator<Item = &CastIntent> {
        self.lowering.cast_intents.iter().filter(move |intent| intent.span == span)
    }

    pub fn cast_intents_for_entry(&self, entry_source_path: Option<&PathBuf>) -> impl Iterator<Item = &CastIntent> {
        self.lowering.cast_intents.iter().filter(move |intent| match (&intent.source_path, entry_source_path) {
            (None, None) => true,
    ...
    pub fn infer_generic_args_from_call_types(&self, item_id: ItemId, arg_types: &[TypeId]) -> Option<Vec<TypeId>> {
