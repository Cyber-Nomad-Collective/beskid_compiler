...
pub fn resolve_host_key(
    ...
    hosts
        .keys()
        .find(|name| {
            name.as_str() == launch_name
                || name.as_str() == tail
                || name.ends_with(&format!(".{tail}"))
        })
        .cloned()
}
...
pub fn build_host_chain<'a>(
    ...
    let launch_key = resolve_host_key(hosts, launched_host).ok_or_else(|| {
        CompositionIssue::UnknownLaunchHost {
...
pub fn merge_host_registries(
    ...
    let scope_name_by_id: HashMap<ScopeId, String> = merged_scopes
        .iter()
        .map(|scope| (scope.id, scope.name.clone()))
        .collect();
    let unified_scope_id: HashMap<String, ScopeId> = merged_scopes
        .iter()
        .map(|scope| (scope.name.clone(), scope.id))
        .collect();
    ...
    for host in chain {
        ...
        for mut reg in regs.iter().cloned() {
            if reg.scope_id != ScopeId::GLOBAL
                && let Some(scope_name) = local_scopes
                    .get(&reg.scope_id)
                    .or_else(|| scope_name_by_id.get(&reg.scope_id))
