...
pub fn evaluate_beskid_code_body(body: &str) -> Result<String, String> {
    let segments = parse_plain_code_body(body).map_err(|err| format!("failed to parse code body: {err:?}"))?;
...
fn eval_code_hole(source: &str) -> Result<String, String> {
    ...
    match &parsed.node {
        Expression::Literal(literal) => match &literal.node.literal.node {
            Literal::String(token) => decode_string_literal_token(token)
                .map_err(|err| format!("invalid string literal in `@{{}}` hole `{source}`: {err:?}")),
            other => Err(format!("unsupported `@{{}}` hole literal (expected string, got {other:?})")),
        },
        _ => Err(format!("unsupported `@{{}}` hole expression `{source}` (AOT mod evaluation not wired)")),
    }
}
...
pub fn code_generate_output(
    ...
    let evaluated_body = if language == "beskid" { evaluate_beskid_code_body(body)? } else { body.to_string() };
    Ok(CodeGenerateOutput { module_path: module_path.to_string(), body: evaluated_body })
}
