...
pub(crate) fn parse_lambda_expression(pair: Pair<Rule>) -> Result<Spanned<Expression>, ParseError> {
    if pair.as_rule() != Rule::LambdaExpression {
        return Err(ParseError::unexpected_rule(pair, Some(Rule::LambdaExpression)));
    }

    let span = SpanInfo::from_span(&pair.as_span());
    let mut inner = pair.into_inner();
    let parameters_pair = inner.next().ok_or(ParseError::missing(Rule::LambdaParameters))?;
    ...
    let lambda = Spanned::new(LambdaExpression { parameters, body: Box::new(body) }, span);
...
fn parse_lambda_parameters(pair: Pair<Rule>) -> Result<Vec<Spanned<LambdaParameter>>, ParseError> {
    if pair.as_rule() != Rule::LambdaParameters {
        return Err(ParseError::unexpected_rule(pair, Some(Rule::LambdaParameters)));
    }
    ...
    match first.as_rule() {
        ...
        _ => Err(ParseError::unexpected_rule(first, Some(Rule::LambdaParameters))),
    }
}

fn parse_lambda_parameter(pair: Pair<Rule>) -> Result<Spanned<LambdaParameter>, ParseError> {
    if pair.as_rule() != Rule::LambdaParameter {
        return Err(ParseError::unexpected_rule(pair, Some(Rule::LambdaParameter)));
    }
    ...
    let (name, ty) = match first.as_rule() {
        ...
        _ => {
            return Err(ParseError::unexpected_rule(first, Some(Rule::LambdaParameter)));
        }
    };
