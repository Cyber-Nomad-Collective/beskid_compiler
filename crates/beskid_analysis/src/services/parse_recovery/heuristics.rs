...
pub(crate) fn can_recover_single_token_deletion(
    ...
    if expected_candidates.is_empty() {
        if matches!(unexpected_class, expected_tokens::ReplacementTokenClass::Delimiter | expected_tokens::ReplacementTokenClass::Operator)
            && inside_nested_structure
            && (next_is_expr_start || matches!(next_class, expected_tokens::ReplacementTokenClass::Delimiter | expected_tokens::ReplacementTokenClass::Operator))
        {
            return true;
        }
        return false;
    ...
    let is_expected_next = expected_candidates.iter().any(|(expected, _, _)| {
        *expected == next_token
            || expected.starts_with(next_token)
            || {
    ...
    let expected_allows_expression_start = expected_candidates.iter().any(|(expected, _, _)| {
        ...
        matches!(
            expected_class,
            expected_tokens::ReplacementTokenClass::Identifier | expected_tokens::ReplacementTokenClass::Number | expected_tokens::ReplacementTokenClass::StringLike
        )
    });
    ...
    if matches!(unexpected_class, expected_tokens::ReplacementTokenClass::Delimiter | expected_tokens::ReplacementTokenClass::Operator)
        ...
        && (expected_allows_expression_start
            ...
            || matches!(next_class, expected_tokens::ReplacementTokenClass::Delimiter | expected_tokens::ReplacementTokenClass::Operator))
    {
        return true;
    }

    if expected_allows_delimiter && next_is_expr_start && matches!(next_class, expected_tokens::ReplacementTokenClass::Delimiter) {
        return true;
    }

    unexpected == next_token && expected_candidates.iter().any(|(expected, _, _)| {
        expected_tokens::replacement_token_class(expected) == expected_tokens::ReplacementTokenClass::Delimiter
            || expected_tokens::replacement_token_class(expected) == expected_tokens::ReplacementTokenClass::Operator
            || matches!(unexpected_class, expected_tokens::ReplacementTokenClass::Delimiter | expected_tokens::ReplacementTokenClass::Operator)
    })
}
    ...
    fn allows_expected_next_token_after_redundant_delimiter() {
        ...
        for source in cases {
            ...
            assert!(
                ...
                "missing next token for case {source}"
            );
