...
fn rejects_explicit_self_parameter_in_impl_method() {
    let pair = crate::surface::util::parse_pair(Rule::Program, "impl Point { i32 len(Point self) { return 0; } }");
    let parsed = beskid_analysis::syntax::Program::parse(pair);
    assert!(parsed.is_err(), "expected explicit self parameter in impl method to be rejected");
}
