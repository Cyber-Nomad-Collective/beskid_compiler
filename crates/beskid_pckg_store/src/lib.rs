...
pub trait AsyncAdministrationRepository: Send + Sync {
    ...
    async fn roles_for_subject(
        &self,
        subject: &str,
    ) -> Result<Vec<AdminRole>, AdministrationStoreError>;
    async fn grant_admin_role(
        &self,
    ...
    async fn list_publisher_verifications(
        &self,
    ) -> Result<Vec<PublisherVerification>, AdministrationStoreError>;
    ...
    async fn list_all_resource_permissions(
        &self,
    ) -> Result<Vec<ResourcePermissionGrant>, AdministrationStoreError>;
    async fn record_package_review(
        &self,
...
pub trait AsyncApiKeyRepository: Send + Sync {
    ...
    async fn find_active_api_key_by_token(
        &self,
        raw_token: &str,
...
impl SqlxPackageRepository {
    ...
    pub async fn migrate_api_keys(&self) -> Result<(), ApiKeyStoreError> {
        sqlx::raw_sql(migrations::CREATE_API_KEYS)
            .execute(&self.pool)
            .await
    ...
    pub async fn bootstrap_super_admin(
        ...
        let timestamp = DateTime::from_timestamp(now_unix_seconds, 0)
...
impl AsyncAdministrationRepository for SqlxPackageRepository {
    async fn list_admin_roles(&self) -> Result<Vec<AdminRoleAssignment>, AdministrationStoreError> {
        let rows = sqlx::query_as::<_, AdminRoleRow>("SELECT subject, role, granted_by_subject, granted_at_utc FROM pckg_admin_roles ORDER BY subject, role")
    ...
    async fn roles_for_subject(
        ...
        validate_administration_subject(subject)?;
        let roles =
            sqlx::query_scalar::<_, String>("SELECT role FROM pckg_admin_roles WHERE subject=$1")
                .bind(subject)
                .fetch_all(&self.pool)
                .await
                .map_err(administration_database_error)?;
        Ok(roles
            .into_iter()
            .filter_map(|role| AdminRole::from_stored(&role))
            .collect())
    }
    ...
    async fn replace_admin_roles(
        ...
        sqlx::query("DELETE FROM pckg_admin_roles WHERE subject=$1")
            ...
            .map_err(administration_database_error)?;
        for role in roles {
            sqlx::query("INSERT INTO pckg_admin_roles (subject,role,granted_by_subject,granted_at_utc) VALUES ($1,$2,$3,$4)").bind(subject).bind(role.stored().expect("validated role")).bind(granted_by_subject).bind(timestamp).execute(&mut *transaction).await.map_err(administration_database_error)?;
        }
        transaction
            .commit()
            .await
    ...
    async fn list_publisher_verifications(
        ...
        rows.into_iter()
            .map(PublisherVerificationRow::into_domain)
            .collect()
    }
    ...
    async fn list_resource_permissions(
        ...
        rows.into_iter()
            .map(ResourcePermissionRow::into_domain)
            .collect()
    }
    ...
    async fn record_package_review(
        ...
        let package_id = Uuid::parse_str(&decision.package_id)
            .map_err(|_| AdministrationStoreError::InvalidPackageId)?;
...
impl AdminRoleRow {
    fn into_domain(self) -> Result<AdminRoleAssignment, AdministrationStoreError> {
        Ok(AdminRoleAssignment {
            ...
            role: AdminRole::from_stored(&self.role)
                .ok_or(AdministrationStoreError::InvalidRole)?,
...
impl AsyncApiKeyRepository for SqlxPackageRepository {
    async fn create_api_key(&self, request: NewApiKey) -> Result<ApiKey, ApiKeyStoreError> {
        ...
        let created_at = DateTime::from_timestamp(request.now_unix_seconds, 0)
            .ok_or(ApiKeyStoreError::InvalidToken)?;
    ...
    async fn revoke_api_key(
        ...
        let revoked_at =
            DateTime::from_timestamp(now_unix_seconds, 0).ok_or(ApiKeyStoreError::InvalidToken)?;
        let changed = sqlx::query("UPDATE pckg_api_keys SET revoked_at_utc=COALESCE(revoked_at_utc,$3) WHERE id=$1 AND subject=$2")
            .bind(id).bind(subject).bind(revoked_at).execute(&self.pool).await.map_err(api_key_database_error)?;
...
fn validate_api_key_scopes(scopes: &[String]) -> Result<(), ApiKeyStoreError> {
    (!scopes.is_empty()
        && scopes
            .iter()
            .all(|scope| matches!(scope.as_str(), "read" | "publish")))
    .then_some(())
