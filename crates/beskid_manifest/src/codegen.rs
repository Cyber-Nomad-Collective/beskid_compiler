...
pub fn render_abi_builtins(manifest: &ManifestRoot) -> String {
    ...
    let mut param_declarations: Vec<_> = param_arrays.iter().collect();
...
mod tests {
    ...
    #[test]
    fn abi_builtin_parameter_declarations_are_deterministically_ordered() {
        let manifest_path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../runtime_manifest.bsol");
        let manifest = crate::load_manifest(&manifest_path).expect("manifest parse");
        let abi = render_abi_builtins(&manifest);
        let declarations: Vec<_> = abi
            .lines()
            .filter_map(|line| line.strip_prefix("const "))
            .map(|line| line.split(':').next().expect("constant name"))
            .collect();
        ...
        assert_eq!(
            declarations, sorted,
            "generated parameter-array declarations must have a canonical order"
        );
    }
}
