...
pub fn generate_host(manifest: &Manifest, out_dir: &Path) -> Result<(), String> {
    ...
    fs::write(out_dir.join("host_handlers.rs"), codegen::render_host_handler_table(manifest))
...
pub fn generate_runtime(manifest: &Manifest, out_dir: &Path, rust_fallback_handlers: bool) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|err| err.to_string())?;
    fs::write(
...
pub fn generate_language(manifest: &Manifest, out_dir: &Path) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("language_handlers.rs"), codegen::render_language_handler_table(manifest))
        .map_err(|err| err.to_string())
}
...
pub fn generate_abi(manifest: &Manifest, out_dir: &Path) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("builtins.rs"), codegen::render_abi_builtins(manifest)).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("symbols.rs"), codegen::render_abi_symbols(manifest)).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("dispatch_tags.rs"), codegen::render_dispatch_tags(manifest))
        .map_err(|err| err.to_string())?;
    fs::write(out_dir.join("dispatch_lookup.rs"), codegen::render_dispatch_lookup(manifest))
        .map_err(|err| err.to_string())?;
    Ok(())
}
...
pub fn generate_jit_registration(manifest: &Manifest, out_dir: &Path) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("kernel_registration.rs"), codegen::render_jit_kernel_registration(manifest))
        .map_err(|err| err.to_string())
}
...
pub fn generate_link_anchor(manifest: &Manifest, out_dir: &Path) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|err| err.to_string())?;
    fs::write(out_dir.join("link_anchor.rs"), codegen::render_link_anchor(manifest)).map_err(|err| err.to_string())
}
...
pub fn generate_analysis_with_v5_intrinsics_from_source(
    ...
    fs::write(out_path, codegen::append_analysis_v5_intrinsics(base, &runtime)).map_err(|err| err.to_string())
}
...
pub fn generate_runtime_handlers(manifest: &Manifest, out_path: &Path) -> Result<(), String> {
    ...
    fs::write(out_path, codegen::render_runtime_handler_specs(manifest)).map_err(|err| err.to_string())
}

/// Convenience: load manifest and write runtime handler metadata.
pub fn generate_runtime_handlers_from_path(manifest_path: &Path, out_path: &Path) -> Result<(), String> {
    let manifest = load_manifest(manifest_path)?;
