...
use beskid_analysis::mod_host::{
    ModHostInput, NativeContractInvoker, StubContractInvoker, extract_mod_contract_registrations_from_syntax,
...
fn sample_mod_rebuild_writes_descriptor_registrations_and_host_dispatches() {
    ...
    assert!(registrations.len() >= 5, "expected non-empty descriptor registrations, got: {registrations:?}");

    let target = beskid_aot::target::detect_target(Some(host_target_triple())).expect("host target");
    let descriptor = build_mod_artifact(ModArtifactBuildRequest {
        artifact: prepare_jit_module(&source_path, &source).expect("lower mod project through syntax codegen"),
    ...
    assert!(descriptor_json.contains("\"registrations\""), "descriptor sidecar must include registrations");
    assert!(
    ...
    assert!(!stub_invoker.invocations().is_empty(), "host must dispatch contracts from rebuilt descriptor");

    let native_invoker = NativeContractInvoker::new(vec![descriptor.object_path()]);
    let program_native = parse_program_with_source_name("Main.bd", host_source).expect("parse host again");
    ...
    assert_eq!(native_generated.session.loaded_descriptor_count(), generated.session.loaded_descriptor_count());
    assert!(!native_invoker.invocations().is_empty(), "native invoker must schedule contracts from descriptor");
}
