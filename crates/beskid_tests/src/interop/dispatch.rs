...
use beskid_abi::{
    RUNTIME_EXPORT_SYMBOLS, SYM_INTEROP_DISPATCH_PTR, SYM_INTEROP_DISPATCH_UNIT, SYM_INTEROP_DISPATCH_USIZE,
    TAG_CHANNEL_CREATE, TAG_STR_LEN,
};
...
fn runtime_exports_include_all_interop_dispatch_symbols() {
    assert!(RUNTIME_EXPORT_SYMBOLS.contains(&SYM_INTEROP_DISPATCH_UNIT), "missing unit interop dispatch symbol export");
    assert!(
        RUNTIME_EXPORT_SYMBOLS.contains(&SYM_INTEROP_DISPATCH_USIZE),
        "missing usize interop dispatch symbol export"
    );
    assert!(RUNTIME_EXPORT_SYMBOLS.contains(&SYM_INTEROP_DISPATCH_PTR), "missing ptr interop dispatch symbol export");
}

#[test]
fn return_group_routing_uses_usize_dispatch_for_string_len_tag() {
    let hello = b"hello";
    let value = BeskidStr { ptr: hello.as_ptr(), len: hello.len() };

    let envelope =
        RuntimeInteropEnvelope { type_desc_ptr: std::ptr::null(), tag: TAG_STR_LEN, pad: 0, payload_ptr: &value };
...
fn colliding_tag_two_routes_by_return_group() {
    ...
    beskid_runtime::run_closure_as_main(|| {
        ...
        assert!(channel_id > 0, "i64 interop must route tag 2 to channel_create, not str_len");
        0
    });
}
...
fn unknown_tag_returns_fallback_for_all_return_groups() {
    let envelope =
        RuntimeInteropEnvelope { type_desc_ptr: std::ptr::null(), tag: 404, pad: 0, payload_ptr: std::ptr::null() };
