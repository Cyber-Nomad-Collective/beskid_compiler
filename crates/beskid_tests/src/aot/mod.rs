...
fn lower_sample_artifact() -> beskid_codegen::CodegenArtifact {
    ...
    let diagnostics =
        run_rules(&program.node, "sample.bd", source, &builtin_rules(), AnalysisOptions::default()).diagnostics;
    assert!(!diagnostics.iter().any(|diag| matches!(diag.severity, Severity::Error)), "expected no analysis errors");
