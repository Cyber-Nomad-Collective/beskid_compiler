...
fn first_two_struct_items(resolution: &beskid_analysis::resolve::Resolution) -> (ItemId, ItemId) {
    let mut ids = resolution.items.iter().filter(|item| item.kind == ItemKind::Type).map(|item| item.id);
    (ids.next().expect("expected first struct"), ids.next().expect("expected second struct"))
}
