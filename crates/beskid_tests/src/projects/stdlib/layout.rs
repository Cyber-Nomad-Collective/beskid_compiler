...
fn checked_in_stdlib_template_has_manifest_and_prelude() {
    ...
    assert!(
        manifest.is_file(),
        "missing stdlib manifest: {}",
        manifest.display()
    );
    assert!(
        prelude.is_file(),
        "missing stdlib prelude: {}",
        prelude.display()
    );
}

#[test]
fn checked_in_stdlib_template_is_resolved_from_corelib_submodule() {
    let root = stdlib_root();
    let normalized = root.to_string_lossy().replace('\\', "/");
    assert!(
        normalized.ends_with("corelib/standard_library"),
        "stdlib root should resolve to corelib/standard_library, got {}",
        root.display()
    );
}

#[test]
fn checked_in_stdlib_template_has_mvp_module_files() {
