...
fn duplicate_local_is_error() {
    ...
    assert!(matches!(errors.first(), Some(ResolveError::DuplicateLocal { .. })));
}
...
fn unknown_value_is_error() {
    ...
    assert!(matches!(errors.first(), Some(ResolveError::UnknownValue { .. })));
}
...
fn unknown_type_is_error() {
    ...
    assert!(matches!(errors.first(), Some(ResolveError::UnknownType { .. })));
}

#[test]
fn shadowing_local_emits_warning() {
    let result =
        resolve_program("unit foo() { let x = 1; if true { let x = 2; } }").expect("expected successful resolution");
    assert!(result.warnings.iter().any(|warning| matches!(warning, ResolveWarning::ShadowedLocal { .. })));
}

#[test]
fn shadowing_item_with_local_emits_warning() {
    let result = resolve_program("unit x() { } unit foo() { let x = 1; }").expect("expected successful resolution");
    assert!(result.warnings.iter().any(|warning| matches!(warning, ResolveWarning::ShadowedLocal { .. })));
}
...
fn qualified_value_path_with_missing_module_is_error() {
    ...
    assert!(errors.iter().any(|error| matches!(error, ResolveError::UnknownModulePath { .. })));
}
...
fn qualified_value_path_with_known_module_and_missing_symbol_is_error() {
    ...
    assert!(errors.iter().any(|error| matches!(error, ResolveError::UnknownValue { .. })));
}
...
fn qualified_type_path_with_known_module_and_missing_symbol_is_error() {
    ...
    assert!(errors.iter().any(|error| matches!(error, ResolveError::UnknownType { .. })));
}
...
fn qualified_module_path_to_private_item_is_error() {
    ...
    assert!(errors.iter().any(|error| matches!(error, ResolveError::PrivateItemInModule { .. })));
}
...
fn non_contract_conformance_target_is_error() {
    ...
    assert!(
        errors.iter().any(|error| matches!(error, ResolveError::InvalidConformanceTarget { .. })),
...
fn syscall_read_builtin_resolves() {
    let result =
        resolve_program("string Main() { return __syscall_read(0, 16); }").expect("expected __syscall_read to resolve");
    assert!(result.warnings.is_empty());
}
...
fn std_panic_resolves() {
    let result = resolve_program("unit Main() { __panic_str(\"boom\"); }").expect("expected __panic_str to resolve");
    assert!(result.warnings.is_empty());
}

#[test]
fn stdarray_new_resolves() {
    let result = resolve_program("i64 Main() { return __array_new(8, 2); }").expect("expected __array_new to resolve");
    assert!(result.warnings.is_empty());
}
...
"#,
    ...
    assert!(result.is_ok(), "use Core.Results should bring Result constructors into scope: {:?}", result.err());
}
