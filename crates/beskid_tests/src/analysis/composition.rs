...
"#;
    ...
    let e1701 = diags
        .iter()
        .find(|d| d.code.as_deref() == Some("E1701"))
        .expect("E1701 diagnostic");
    assert!(
        !e1701.span.is_empty(),
        "missing launch host should anchor to program span"
    );
}
...
"#;
    ...
    let e1709 = diags
        .iter()
        .find(|d| d.code.as_deref() == Some("E1709"))
        .expect("E1709 diagnostic");
...
"#;
    let codes = composition_codes(source);
    assert!(
        codes.contains("E1702"),
        "expected E1702 for duplicate launch, got {codes:?}"
    );
    let second_launch = source.rfind("launch").expect("second launch");
    let diags = analyze_composition(source);
    let e1702 = diags
        .iter()
        .find(|d| d.code.as_deref() == Some("E1702"))
        .expect("E1702 diagnostic");
    let span_start = e1702.span.offset();
    let span_end = span_start.saturating_add(e1702.span.len().max(1));
    assert!(
        span_start <= second_launch && second_launch < span_end,
        "E1702 should anchor to the duplicate launch"
    );
}
...
"#;
    let codes = composition_codes(source);
    assert!(
        codes.contains("E1703"),
        "expected dependency cycle E1703, got {codes:?}"
    );
}
