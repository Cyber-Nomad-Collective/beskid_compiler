...
fn concurrency_source_and_materialized_parse_equally() {
    ...
    let materialized_source = std::fs::read_to_string(&materialized).expect("read materialized Concurrency.bd");
    assert_eq!(source, materialized_source, "materialized Concurrency.bd must match package source verbatim");
