...
use crate::projects::fixture_harness::{
    ...
    resolve_corelib_tests_entry_with_assembly, with_large_test_stack, with_project_test_env,
};
...
use beskid_queries::{
    AstNodeId, AstNodeKey, IndexedNodeKind, ItemSignature, SemanticTypeId, SourceUnitId,
    build_typed_program, child_nodes, generic_call_specialization, item_abi_signature, item_name,
    node_kind, project_session_for_syntax_assembly, reachable_items, with_db,
};
...
fn syscall_result_predicates_have_call_derived_pointer_specializations() {
    with_large_test_stack(|| {
        ...
        with_project_test_env(&root, || {
            ...
            with_db(|db| {
                let syntax_assembly = Arc::new(SyntaxProgramAssembly::from(
                ...
                assert_eq!(
                    matching, 4,
                    "each Results.IsOk call must retain POINTER -> BOOL"
                );
                ...
                let reachable = reachable_items(db, root, entry)
                    .expect("reachable item query")
                    .expect("reachable item facts");
                ...
                assert!(
                    missing.is_empty(),
                    "generic items without specialization: {missing:?}"
                );
            });
        });
    });
