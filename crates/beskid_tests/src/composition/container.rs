...
fn two_scopes_plural_inject_reverse_dispose() {
    ...
    let consumer_pos = scope1_disposes
        .iter()
        .position(|line| line.as_str() == "dispose:consumer");
    let handler1_pos = scope1_disposes
        .iter()
        .position(|line| line.as_str() == "dispose:handler1");
    let handler2_pos = scope1_disposes
        .iter()
        .position(|line| line.as_str() == "dispose:handler2");
    ...
    let last_disposes: Vec<&str> = entries
        .iter()
        .rev()
...
fn unknown_registration_resolves_to_error() {
    ...
    assert!(matches!(
        err,
        beskid_runtime::composition::ContainerError::UnknownRegistration(_)
    ));
    container.shutdown().expect("shutdown");
}
...
fn extern_c_abi_roundtrip_through_builtins() {
    ...
    let rc = composition_bind_plural(
        container,
        20,
        plural_targets.as_ptr(),
        plural_targets.len() as i64,
    );
    assert_eq!(rc, 0);
