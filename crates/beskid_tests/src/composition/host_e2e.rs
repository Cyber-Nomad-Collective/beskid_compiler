...
use beskid_runtime::composition::{
    Lifetime, RegistrationId, RegistrationRecord, RuntimeContainer, ScopeId,
};
...
fn host_with_two_scopes_plural_inject_reverse_dispose() {
    ...
    container
        ...
        .expect("enter request scope");
    let plural = container
        .resolve_plural(RegistrationId(12))
        .expect("resolve plural Middleware");
    assert_eq!(plural.len(), 2);
    container
        .resolve(RegistrationId(12))
        .expect("resolve Handler");

    container
        .enter_scope(session_scope)
        .expect("enter session scope");
    container
        .resolve(RegistrationId(20))
        .expect("resolve SessionStore");

    container
        .leave_scope(session_scope)
        .expect("leave session scope");
    container
        .leave_scope(request_scope)
        .expect("leave request scope");
    container.shutdown().expect("shutdown");

    let entries = log.borrow().clone();
    // 1) every scope-owned service must run init before any dispose
    let first_init = entries
        .iter()
        .position(|l| l.starts_with("init:"))
        .expect("at least one init entry");
    let first_dispose = entries
        .iter()
        .position(|l| l.starts_with("dispose:"))
        .expect("at least one dispose entry");
    assert!(
        first_init < first_dispose,
        "init must precede dispose: {entries:?}"
    );

    // 2) session disposes before request leave
    let session_pos = entries
        .iter()
        .position(|l| l == "dispose:SessionStore")
        .expect("session dispose recorded");
    let auth_pos = entries
        .iter()
        .position(|l| l == "dispose:MiddlewareAuth")
        .expect("auth dispose recorded");
    let metrics_pos = entries
        .iter()
        .position(|l| l == "dispose:MiddlewareMetrics")
        .expect("metrics dispose recorded");
    assert!(
    ...
    let handler_pos = entries
        .iter()
        .position(|l| l == "dispose:Handler")
    ...
    let logger_pos = entries
        .iter()
        .rposition(|l| l == "dispose:Logger")
        .expect("logger dispose recorded");
    assert!(
