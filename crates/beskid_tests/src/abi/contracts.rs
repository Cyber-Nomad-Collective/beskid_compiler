...
use beskid_abi::{
    BESKID_RUNTIME_ABI_VERSION, BUILTIN_SPECS, BeskidArray, BeskidStr, DispatchReturnGroup, RUNTIME_EXPORT_SYMBOLS,
    ...
    SYM_RUNTIME_PREEMPT_CHECK, SYM_STR_LEN, SYM_SYSCALL_READ, SYM_SYSCALL_WRITE, TAG_FIBER_PROCESSOR_COUNT,
    TAG_FIBER_SPAWN_WITH_CANCEL_SLOT, TAG_FS_WRITE_TEXT, dispatch_route_for_symbol, is_dispatch_symbol,
};
...
fn corelib_i64_constant(source: &str, name: &str) -> i64 {
    // Parses discriminants from ToI64 match arms: `Status::Name => N,`
    let pattern = format!("::{} => ", name);
    let start = source.find(&pattern).unwrap_or_else(|| panic!("missing corelib i64 constant `{name}` in ToI64"));
    let after_arrow = &source[start + pattern.len()..];
    let end = after_arrow.find(',').unwrap_or_else(|| panic!("missing comma after value for `{name}`"));
    after_arrow[..end]
        .trim()
        .parse::<i64>()
...
fn concurrency_corelib_status_codes_match_runtime_contract() {
    ...
    assert_eq!(corelib_i64_constant(&status, "Ok"), beskid_runtime::status::STATUS_OK);
    assert_eq!(corelib_i64_constant(&status, "Closed"), beskid_runtime::status::STATUS_CLOSED);
    assert_eq!(corelib_i64_constant(&status, "Cancelled"), beskid_runtime::status::STATUS_CANCELLED);
    assert_eq!(corelib_i64_constant(&status, "WouldBlock"), beskid_runtime::status::STATUS_WOULD_BLOCK);
    assert_eq!(corelib_i64_constant(&status, "HubEmpty"), beskid_runtime::status::STATUS_HUB_EMPTY);
    assert_eq!(corelib_i64_constant(&status, "HubLimit"), beskid_runtime::status::STATUS_HUB_LIMIT);
    assert_eq!(corelib_i64_constant(&status, "HubNotFound"), beskid_runtime::status::STATUS_HUB_NOT_FOUND);
    // MutexBusy (value 1) is now Status::Closed in the discriminated union;
    // mutex TryLock maps via Status.FromI64 and treats non-Ok as would-block.

    assert_eq!(corelib_i64_constant(&fiber_join_status, "Ok"), beskid_runtime::status::FIBER_JOIN_OK);
    assert_eq!(corelib_i64_constant(&fiber_join_status, "Cancelled"), beskid_runtime::status::FIBER_JOIN_CANCELLED);
    assert_eq!(corelib_i64_constant(&fiber_join_status, "Panicked"), beskid_runtime::status::FIBER_JOIN_PANICKED);
    assert_eq!(
        corelib_i64_constant(&fiber_join_status, "StackOverflow"),
        beskid_runtime::status::FIBER_JOIN_STACK_OVERFLOW
    );
    assert_eq!(corelib_i64_constant(&fiber_join_status, "NotDone"), beskid_runtime::status::FIBER_JOIN_NOT_DONE);
}

#[test]
fn fiber_dispatch_tags_cover_processor_count_and_cancel_slot_spawn() {
    let processor =
        dispatch_route_for_symbol("fiber_processor_count").expect("fiber_processor_count should be a dispatch symbol");
...
fn mvp_corelib_ops_route_through_dispatch_or_kernel() {
    assert!(is_dispatch_symbol(SYM_STR_LEN), "`str_len` should be a dispatch symbol in ABI v4");
    assert!(is_dispatch_symbol(SYM_SYSCALL_WRITE), "`syscall_write` should be a dispatch symbol in ABI v4");
    assert!(is_dispatch_symbol(SYM_SYSCALL_READ), "`syscall_read` should be a dispatch symbol in ABI v4");
    assert!(
        RUNTIME_EXPORT_SYMBOLS.contains(&SYM_INTEROP_DISPATCH_USIZE),
        "kernel must export usize dispatch entrypoint"
    );
    assert!(RUNTIME_EXPORT_SYMBOLS.contains(&SYM_INTEROP_DISPATCH_PTR), "kernel must export ptr dispatch entrypoint");
}

#[test]
fn host_ops_are_not_kernel_exports_v4() {
    for symbol in ["fs_write_text", "fs_read_text", "env_get", "env_set", "process_getpid", "tty_winsize"] {
        assert!(!RUNTIME_EXPORT_SYMBOLS.contains(&symbol), "host op `{symbol}` must not be a kernel export in ABI v4");
    }
}
...
fn host_manifest_entries_match_generated_registration_table() {
    ...
    let host_handlers = std::fs::read_to_string(crate_dir.join("../beskid_host/src/generated/host_handlers.rs"))
        .expect("generated host handler table should be readable");

    let dispatch_fn_count = host_handlers.matches("unsafe extern \"C\" fn host_dispatch_").count();
    let registration_count = host_handlers.matches("HandlerTableEntry {").count();

    assert!(registration_count > 0, "host handler registration table must not be empty");
    assert_eq!(
...
fn minimal_profile_host_tag_traps_without_registration() {
    let _ = beskid_runtime::beskid_register_handlers(u64::from(BESKID_RUNTIME_ABI_VERSION), std::ptr::null(), 0);
...
fn runtime_array_len_matches_array_new_length() {
    with_runtime_scope(|_, _| {
        ...
        assert_eq!(array_len(ptr), 3, "array_len(array_new(8, 3)) should report logical length 3");
    });
}
