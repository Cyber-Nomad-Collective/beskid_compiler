...
use beskid_analysis::services::{FrontEndOptions, ResolvedInput, resolved_input_from_plan, synthetic_compile_plan_for_source};
use beskid_codegen::lower_prepared_syntax_entrypoint;
...
fn prepared_syntax_entrypoint_lowers_without_hir_host_authority() {
    ...
    let isa = isa::lookup_by_name("x86_64").expect("x86 ISA")
        .finish(settings::Flags::new(settings::builder())).expect("finish ISA");
    let lowered = with_db(|db| lower_prepared_syntax_entrypoint(db, &front, "Main", target, isa.as_ref()))
