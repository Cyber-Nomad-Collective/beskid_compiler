use super::support::{
    ...
    SyntaxGenerationId, SyntaxModuleItem, TargetMetadata, build_typed_program,
    build_typed_program_with_corelib_services, call_lowering, canonical_corelib_service_capability,
    canonical_corelib_service_source_path, canonical_corelib_service_sources, default_libcall_names,
    emit_syntax_program, find_call_expression, find_definition_of_kind, find_function_definition,
    find_function_definitions, find_node, find_test_definition, isa, item_fixture_with_root, item_name,
    lower_syntax_program, parse_program_with_source_name, settings, typed_array_allocation,
};
...
#[test]
fn canonical_array_empty_specialization_allocates_from_its_concrete_element_descriptor() {
    ...
    let application_source = "use Core.Collections.Array; i64[] Main() { return Array.Empty<i64>(); }";
    ...
    let main = find_function_definition(input.database(), application_root).expect("application Main");
