...
use beskid_queries::{AggregateFieldShape, AstNodeKey, SemanticTypeId, aggregate_layout, aggregate_literal_declaration};
...
pub fn emit_aggregate_static_data<M: Module>(
    ...
    if plan.pointer_map_offsets.is_empty() { pointer_map_bytes.extend_from_slice(&0u64.to_le_bytes()); }
    else { for offset in plan.pointer_map_offsets.iter() { pointer_map_bytes.extend_from_slice(&offset.to_le_bytes()); } }
...
fn write_word(bytes: &mut [u8], offset: usize, value: u64) -> Result<(), ModuleError> {
    let destination = bytes.get_mut(offset..offset + 8).ok_or_else(|| ModuleError::Backend(anyhow::anyhow!("aggregate static-data layout overflow")))?;
...
impl CodegenInput<'_> {
    pub fn aggregate_static_plan(&self, literal: AstNodeKey) -> Option<AggregateStaticPlan> {
        ...
        if header.size < 16 || !valid_alignment(header.alignment) || descriptor.size != 40 || descriptor.alignment != 8 || request.size != 24 || request.alignment != 8 { return None; }
        let mut size = header.size; let mut alignment = header.alignment; let mut pointer_map_offsets = Vec::new(); let mut fields = Vec::with_capacity(aggregate.fields.len());
        for (_, shape) in aggregate.fields.iter() {
            let abi_type = match shape { AggregateFieldShape::Scalar(semantic) => *semantic, AggregateFieldShape::Nominal(_) => SemanticTypeId::POINTER };
            let (field_size, field_alignment, pointer) = scalar_layout(self.target().pointer_width, abi_type)?;
            size = align_to(size, field_alignment)?; let field_offset = size; size = size.checked_add(field_size)?; alignment = alignment.max(field_alignment);
            if pointer { pointer_map_offsets.push(field_offset); }
            fields.push(AggregateStaticField { abi_type, field_offset });
        }
        let object_size = align_to(size, alignment)?;
        let unit = self.typed_program().assembly.units().iter().position(|unit| paths_match(&unit.path, literal.unit.path(self.database())))?;
        let identity = format!("u{unit}_g{}_n{}", literal.generation.0, literal.node.0);
        Some(AggregateStaticPlan { literal, descriptor_symbol: format!("__beskid_aggregate_descriptor_{identity}"), pointer_map_symbol: format!("__beskid_aggregate_pointer_map_{identity}"), allocation_request_symbol: format!("__beskid_aggregate_allocation_request_{identity}"), object_size, object_alignment: alignment, pointer_map_offsets: pointer_map_offsets.into(), fields: fields.into() })
    }
}

fn scalar_layout(pointer_width: u8, ty: SemanticTypeId) -> Option<(u64, u64, bool)> { let pointer = u64::from(pointer_width.checked_div(8)?); match ty { SemanticTypeId::BOOL | SemanticTypeId::U8 => Some((1, 1, false)), SemanticTypeId::I32 | SemanticTypeId::CHAR => Some((4, 4, false)), SemanticTypeId::I64 | SemanticTypeId::F64 => Some((8, 8, false)), SemanticTypeId::WORD => Some((pointer, pointer, false)), SemanticTypeId::POINTER | SemanticTypeId::STRING => Some((pointer, pointer, true)), _ => None } }
fn valid_alignment(value: u64) -> bool { value.is_power_of_two() && value > 0 }
fn align_to(value: u64, alignment: u64) -> Option<u64> { valid_alignment(alignment).then_some(())?; value.checked_add(alignment - 1).map(|value| value & !(alignment - 1)) }
fn paths_match(left: &std::path::Path, right: &std::path::Path) -> bool { left.canonicalize().unwrap_or_else(|_| left.to_path_buf()) == right.canonicalize().unwrap_or_else(|_| right.to_path_buf()) }
