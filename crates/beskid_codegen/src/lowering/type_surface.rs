...
use beskid_analysis::types::{TypeResult, UnitTypeSurface, contract_signatures_for_types, merge_unit_surfaces};
...
pub(crate) fn contract_method_order(type_result: &TypeResult) -> HashMap<ItemId, Vec<String>> {
    merge_unit_surfaces(
        type_result.unit_surfaces.iter().map(|(k, v)| (k.clone(), v.clone())),
        std::sync::Arc::new(UnitTypeSurface::default()),
    )
    .contract_method_order
}

pub(crate) fn contract_signatures(type_result: &TypeResult) -> HashMap<(ItemId, String), FunctionSignature> {
...
pub(crate) fn struct_event_fields(type_result: &TypeResult) -> &HashMap<ItemId, HashMap<String, Option<usize>>> {
