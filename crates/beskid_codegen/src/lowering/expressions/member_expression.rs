...
impl Lowerable<NodeLoweringContext<'_, '_>> for HirMemberExpression {
    ...
    fn lower(node: &Spanned<Self>, ctx: &mut NodeLoweringContext<'_, '_>) -> Result<Self::Output, CodegenError> {
        let target_value = lower_node(&node.node.target, ctx)?
            .ok_or(CodegenError::UnsupportedNode { span: node.node.target.span, node: "unit-valued member target" })?;
        let mut target_type = ctx.require_expr_type_for_node(&node.node.target)?;
        target_type = resolve_monomorph_type_id(ctx.type_result, &ctx.codegen.active_generic_substitution, target_type);
        let field_name = node.node.member.node.name.as_str();
        if field_name == "handle" && is_fiber_handle_type(ctx.type_result, ctx.resolution, target_type) {
            return Ok(Some(target_value));
        }
        let item_id = struct_item_id(ctx.type_result, target_type)
            .ok_or(CodegenError::UnsupportedNode { span: node.node.target.span, node: "member target type" })?;
        let offsets =
            struct_field_offsets(ctx.resolution, ctx.type_result, item_id, ctx.codegen.current_source_path.as_ref())
                .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "member offsets" })?;
        let offset = offsets
            .get(field_name)
            .copied()
            .ok_or(CodegenError::UnsupportedNode { span: node.node.member.span, node: "member offset" })?;
        let field_type = ctx
            .type_result
            ...
            .map(|(_, ty)| *ty)
            .ok_or(CodegenError::UnsupportedNode { span: node.node.member.span, node: "member field type" })?;
        let clif_ty = map_type_id_to_clif(ctx.type_result, field_type)
            .ok_or(CodegenError::UnsupportedNode { span: node.node.member.span, node: "member field clif type" })?;
