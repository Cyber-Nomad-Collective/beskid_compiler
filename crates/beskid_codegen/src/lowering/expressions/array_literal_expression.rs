...
use cranelift_codegen::ir::{AbiParam, ExtFuncData, ExternalName, InstBuilder, MemFlags, Signature, Value};
...
impl Lowerable<NodeLoweringContext<'_, '_>> for HirArrayLiteralExpression {
    ...
    fn lower(node: &Spanned<Self>, ctx: &mut NodeLoweringContext<'_, '_>) -> Result<Self::Output, CodegenError> {
        ...
        let layout = ctx
            ...
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "array element layout" })?;
        ...
        let data_ptr = ctx.builder.ins().load(pointer_type(), MemFlags::new(), array_ptr, 0);
        ...
        for (i, element) in node.node.elements.iter().enumerate() {
            let value = lower_node(element, ctx)?
                .ok_or(CodegenError::UnsupportedNode { span: element.span, node: "unit-valued array element" })?;

            let offset = ctx.builder.ins().iconst(pointer_type(), (i * elem_size) as i64);
...
fn emit_array_new(
    ...
    let call = ctx.builder.ins().call(func_ref, &[elem_size_val, count_val]);
    let result = ctx
        .builder
        .inst_results(call)
        .first()
        .copied()
        .ok_or(CodegenError::UnsupportedNode { span, node: "array_new result" })?;
    Ok(result)
}
