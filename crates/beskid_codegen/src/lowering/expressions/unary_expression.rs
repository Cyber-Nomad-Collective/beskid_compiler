...
impl Lowerable<NodeLoweringContext<'_, '_>> for HirUnaryExpression {
    ...
    fn lower(node: &Spanned<Self>, ctx: &mut NodeLoweringContext<'_, '_>) -> Result<Self::Output, CodegenError> {
        let value = lower_node(&node.node.expr, ctx)?
            .ok_or(CodegenError::UnsupportedNode { span: node.node.expr.span, node: "unit-valued unary operand" })?;
        ...
        let clif_ty = map_type_id_to_clif(ctx.type_result, type_id)
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "unary operand type" })?;
        ...
        let lowered = match node.node.op.node {
            HirUnaryOp::Neg => {
                if clif_ty.is_float() {
                    ...
                    return Err(CodegenError::UnsupportedNode { span: node.span, node: "unary negation type" });
                }
            }
            HirUnaryOp::Not => {
                let clif_is_bool = clif_ty.is_int() && clif_ty.bits() == 8;
                if matches!(type_info, Some(TypeInfo::Primitive(HirPrimitiveType::Bool))) || clif_is_bool {
                    let one = ctx.builder.ins().iconst(types::I8, 1);
                    ctx.builder.ins().bxor(value, one)
                } else {
                    return Err(CodegenError::UnsupportedNode { span: node.span, node: "unary not type" });
                }
            }
        };
