...
use crate::lowering::descriptor::{enum_payload_start, enum_variant_field_offsets, is_pointer_like_type};
use crate::lowering::locals::{node_expr_type, type_id_for_item};
use crate::lowering::lowerable::{Lowerable, lower_node};
use crate::lowering::node_context::NodeLoweringContext;
use crate::lowering::types::{map_type_id_to_clif, pointer_type, resolve_type_path_item_id_for_codegen};
...
impl Lowerable<NodeLoweringContext<'_, '_>> for HirEnumConstructorExpression {
    ...
    fn lower(node: &Spanned<Self>, ctx: &mut NodeLoweringContext<'_, '_>) -> Result<Self::Output, CodegenError> {
        ...
        let type_id = enum_layout_type_id(ctx, node, inferred_type_id)
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "enum constructor type" })?;
        let item_id = enum_base_item_id(ctx, type_id, node)?;
        let layout = ctx
            .codegen
            .type_layout(ctx.type_result, type_id)
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "enum constructor layout" })?;
        let offsets = enum_variant_field_offsets(ctx.type_result, item_id, variant_name)
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "enum constructor offsets" })?;
        ...
        let variants = ctx
            ...
            .get(&item_id)
            .ok_or(CodegenError::UnsupportedNode { span: node.span, node: "enum variants" })?;
        let (tag, field_types) = variants
            .iter()
            .enumerate()
            .find(|(_, (name, _))| name == variant_name)
            .map(|(index, (_, fields))| (index as i64, fields))
            .ok_or(CodegenError::UnsupportedNode { span: node.node.path.span, node: "enum variant lookup" })?;
        ...
        for ((arg, field_type), offset) in node.node.args.iter().zip(field_types.iter()).zip(offsets) {
            let mut value = lower_node(arg, ctx)?
                .ok_or(CodegenError::UnsupportedNode { span: arg.span, node: "unit-valued enum argument" })?;
            let value_ty = ctx.builder.func.dfg.value_type(value);
            let store_ty = map_type_id_to_clif(ctx.type_result, *field_type)
                .ok_or(CodegenError::UnsupportedNode { span: arg.span, node: "enum field clif type" })?;
            ...
            let _store_ty = map_type_id_to_clif(ctx.type_result, *field_type)
                .ok_or(CodegenError::UnsupportedNode { span: arg.span, node: "enum field clif type" })?;
...
fn enum_type_candidate(
    ...
    matches!(type_result.types.get(type_id), Some(TypeInfo::Named(_) | TypeInfo::Applied { .. }))
}
...
fn enum_layout_type_id(
    ...
    if matches!(ctx.type_result.types.get(inferred), Some(TypeInfo::Named(_) | TypeInfo::Applied { .. })) {
...
fn enum_base_item_id(
    ...
    let segments: Vec<String> =
        node.node.path.node.type_path.node.segments.iter().map(|segment| segment.node.name.node.name.clone()).collect();
...
fn enum_constructor_type_from_context(
    ...
    let segments: Vec<String> =
        node.node.path.node.type_path.node.segments.iter().map(|segment| segment.node.name.node.name.clone()).collect();
    let base_item_id = resolve_type_path_item_id_for_codegen(ctx.resolution, ctx.type_result, &segments)?;
