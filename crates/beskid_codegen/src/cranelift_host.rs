...
/// Declare every entry in [`beskid_abi::BUILTIN_SPECS`] plus event symbols as import functions on `module`.
...
pub fn declare_user_functions<M: Module>(
    ...
    for function in &artifact.functions {
        let func_id =
            module.declare_function(&function.name, linkage, &function.function.signature)?;
        func_ids.insert(function.name.clone(), func_id);
        declared.push(function.name.clone());
    }
    Ok(declared)
}
