...
fn perf_smoke_aot_build_and_run_stay_within_time_budget() {
    ...
    let run = run_binary(&output_binary, Duration::from_millis(run_max_ms.saturating_mul(2)));
...
fn perf_batch_release_builds_stay_within_total_budget() {
    let cli = BeskidCliInvoker::new();
    let fixtures = ["smoke_project", "contracts_dispatch", "method_dispatch", "closure_capture"];
...
fn read_budget_ms(env_name: &str, fallback: u64) -> u64 {
    std::env::var(env_name).ok().and_then(|value| value.parse::<u64>().ok()).unwrap_or(fallback)
}
