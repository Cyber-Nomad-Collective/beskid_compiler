...
pub(crate) async fn list_docs(
    ...
    match browser.list_docs() {
        Ok(entries) => {
            Json(entries.into_iter().map(entry_response).collect::<Vec<_>>()).into_response()
        }
...
pub(crate) async fn structured_docs(
    ...
    match browser.documentation() {
        Ok(documentation) => Json(StructuredDocumentationResponse {
...
async fn browser_for_request(
    state: &AppState,
    headers: &HeaderMap,
    path: &ArtifactPath,
) -> Option<ArtifactBrowser> {
...
async fn resolve_version(
    ...
    if requested.eq_ignore_ascii_case("latest") {
        ...
        let version = state
            .packages
            .find_version(&package.id, requested)
            .await
            .ok()??;
...
fn text_response(
    result: Result<String, beskid_pckg_artifacts::ArtifactError>,
