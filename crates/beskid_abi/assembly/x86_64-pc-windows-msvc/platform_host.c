...
static __declspec(noreturn) void beskid_args_trap(uint8_t code, const char *message, size_t message_len) {
    beskid_rt_v5_trap(code, (void *)message, message_len);
}

#define BESKID_ARGS_TRAP(code, message) beskid_args_trap((code), (message), sizeof(message) - 1)
