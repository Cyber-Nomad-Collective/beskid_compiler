...
void beskid_rt_v5_args_handoff_utf8(int64_t argc, const char *const *argv) {
    ...
    for (int64_t i = 0; i < argc; ++i) {
        if (argv[i] == NULL) beskid_args_trap(10, "Core.Args handoff is invalid");
        size_t len = __builtin_strlen(argv[i]);
        if (len > SIZE_MAX - bytes) beskid_args_trap(5, "Core.Args storage allocation failed");
        bytes += len;
    }
    if (headers > SIZE_MAX - bytes) beskid_args_trap(5, "Core.Args storage allocation failed");
    size_t total = headers + bytes;
    if (total == 0) total = 1;
    unsigned char *storage = mmap(NULL, total, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
struct BeskidStr *beskid_rt_v5_args_get(int64_t index) {
    if (index < 0 || index >= beskid_args.count) beskid_args_trap(2, "Core.Args argument index is out of range");
...
void *beskid_rt_v5_intrinsic_guarded_stack_allocate(size_t usable_size) {
    ...
    void *reservation = mmap(NULL, total_size, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
