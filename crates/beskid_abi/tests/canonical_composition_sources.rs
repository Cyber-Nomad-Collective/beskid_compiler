...
fn canonical_composition_owns_a_frozen_plan_container() {
    ...
    assert!(
        !composition.contains("pub bool CompositionRegister(pointer container, pointer service) { return false; }")
    );
}
...
fn canonical_composition_admits_only_validated_plan_entries_before_activation() {
    ...
    assert!(
        composition
            .contains("if CompositionContainerValid(container) == 0 || service == NativePointer(0) { return false; }")
    );
