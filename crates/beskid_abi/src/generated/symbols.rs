// @generated by beskid_manifest from runtime_manifest.toml — do not edit.

pub const SYM_ALLOC: &str = "alloc";
/// Legacy runtime symbol for dispatch tag `ArrayLen` (array_len)
pub const SYM_ARRAY_LEN: &str = "array_len";
/// Legacy runtime symbol for dispatch tag `ArrayNew` (array_new)
pub const SYM_ARRAY_NEW: &str = "array_new";
pub const SYM_BESKID_REGISTER_CALLBACKS: &str = "beskid_register_callbacks";
pub const SYM_BESKID_REGISTER_HANDLERS: &str = "beskid_register_handlers";
pub const SYM_ABI_VERSION: &str = "beskid_runtime_abi_version";
/// Legacy runtime symbol for dispatch tag `BytesCompare` (bytes_compare)
pub const SYM_BYTES_COMPARE: &str = "bytes_compare";
/// Legacy runtime symbol for dispatch tag `BytesCopy` (bytes_copy)
pub const SYM_BYTES_COPY: &str = "bytes_copy";
/// Legacy runtime symbol for dispatch tag `BytesFromStr` (bytes_from_str)
pub const SYM_BYTES_FROM_STR: &str = "bytes_from_str";
/// Legacy runtime symbol for dispatch tag `BytesGet` (bytes_get)
pub const SYM_BYTES_GET: &str = "bytes_get";
/// Legacy runtime symbol for dispatch tag `BytesSet` (bytes_set)
pub const SYM_BYTES_SET: &str = "bytes_set";
/// Legacy runtime symbol for dispatch tag `ChannelClose` (channel_close)
pub const SYM_CHANNEL_CLOSE: &str = "channel_close";
/// Legacy runtime symbol for dispatch tag `ChannelCreate` (channel_create)
pub const SYM_CHANNEL_CREATE: &str = "channel_create";
/// Legacy runtime symbol for dispatch tag `ChannelReceivePtr` (channel_receive_ptr)
pub const SYM_CHANNEL_RECEIVE_PTR: &str = "channel_receive_ptr";
/// Legacy runtime symbol for dispatch tag `ChannelReceiveStatus` (channel_receive_status)
pub const SYM_CHANNEL_RECEIVE: &str = "channel_receive_status";
/// Legacy runtime symbol for dispatch tag `ChannelReceiveValue` (channel_receive_value)
pub const SYM_CHANNEL_RECEIVE_VALUE: &str = "channel_receive_value";
/// Legacy runtime symbol for dispatch tag `ChannelSend` (channel_send)
pub const SYM_CHANNEL_SEND: &str = "channel_send";
/// Legacy runtime symbol for dispatch tag `ChannelSendPtr` (channel_send_ptr)
pub const SYM_CHANNEL_SEND_PTR: &str = "channel_send_ptr";
/// Legacy runtime symbol for dispatch tag `ChannelTryReceive` (channel_try_receive)
pub const SYM_CHANNEL_TRY_RECEIVE: &str = "channel_try_receive";
/// Legacy runtime symbol for dispatch tag `ChannelTryReceivePtr` (channel_try_receive_ptr)
pub const SYM_CHANNEL_TRY_RECEIVE_PTR: &str = "channel_try_receive_ptr";
/// Legacy runtime symbol for dispatch tag `ChannelTrySend` (channel_try_send)
pub const SYM_CHANNEL_TRY_SEND: &str = "channel_try_send";
/// Legacy runtime symbol for dispatch tag `ChannelTrySendPtr` (channel_try_send_ptr)
pub const SYM_CHANNEL_TRY_SEND_PTR: &str = "channel_try_send_ptr";
/// Legacy runtime symbol for dispatch tag `ClockMonotonicNanos` (clock_monotonic_nanos)
pub const SYM_CLOCK_MONOTONIC_NANOS: &str = "clock_monotonic_nanos";
/// Legacy runtime symbol for dispatch tag `ClockRealtimeNanos` (clock_realtime_nanos)
pub const SYM_CLOCK_REALTIME_NANOS: &str = "clock_realtime_nanos";
pub const SYM_COMPOSITION_BIND_PLURAL: &str = "composition_bind_plural";
pub const SYM_COMPOSITION_CONTAINER_CREATE: &str = "composition_container_create";
pub const SYM_COMPOSITION_CONTAINER_DROP: &str = "composition_container_drop";
pub const SYM_COMPOSITION_LAUNCH: &str = "composition_launch";
pub const SYM_COMPOSITION_REGISTER: &str = "composition_register";
pub const SYM_COMPOSITION_RESOLVE: &str = "composition_resolve";
pub const SYM_COMPOSITION_RESOLVE_PLURAL: &str = "composition_resolve_plural";
pub const SYM_COMPOSITION_SCOPE_DEPTH: &str = "composition_scope_depth";
pub const SYM_COMPOSITION_SCOPE_ENTER: &str = "composition_scope_enter";
pub const SYM_COMPOSITION_SCOPE_LEAVE: &str = "composition_scope_leave";
pub const SYM_COMPOSITION_SHUTDOWN: &str = "composition_shutdown";
pub const SYM_DYNAMIC_CAST_CHECKED: &str = "dynamic_cast_checked";
pub const SYM_DYNAMIC_CELL_CREATE: &str = "dynamic_cell_create";
pub const SYM_DYNAMIC_CELL_WRAP: &str = "dynamic_cell_wrap";
pub const SYM_DYNAMIC_MAP_AOT: &str = "dynamic_map_aot";
pub const SYM_DYNAMIC_MAP_FALLBACK: &str = "dynamic_map_fallback";
pub const SYM_DYNAMIC_OBJECT_ALLOC: &str = "dynamic_object_alloc";
/// Legacy runtime symbol for dispatch tag `EnvGet` (env_get)
pub const SYM_ENV_GET: &str = "env_get";
/// Legacy runtime symbol for dispatch tag `EnvGetcwd` (env_getcwd)
pub const SYM_ENV_GETCWD: &str = "env_getcwd";
/// Legacy runtime symbol for dispatch tag `EnvSet` (env_set)
pub const SYM_ENV_SET: &str = "env_set";
/// Legacy runtime symbol for dispatch tag `EventGetHandler` (event_get_handler)
pub const SYM_EVENT_GET_HANDLER: &str = "event_get_handler";
/// Legacy runtime symbol for dispatch tag `EventLen` (event_len)
pub const SYM_EVENT_LEN: &str = "event_len";
/// Legacy runtime symbol for dispatch tag `EventSubscribe` (event_subscribe)
pub const SYM_EVENT_SUBSCRIBE: &str = "event_subscribe";
/// Legacy runtime symbol for dispatch tag `EventUnsubscribeFirst` (event_unsubscribe_first)
pub const SYM_EVENT_UNSUBSCRIBE_FIRST: &str = "event_unsubscribe_first";
/// Legacy runtime symbol for dispatch tag `FiberCancel` (fiber_cancel)
pub const SYM_FIBER_CANCEL: &str = "fiber_cancel";
/// Legacy runtime symbol for dispatch tag `FiberCurrentId` (fiber_current_id)
pub const SYM_FIBER_CURRENT_ID: &str = "fiber_current_id";
/// Legacy runtime symbol for dispatch tag `FiberDetach` (fiber_detach)
pub const SYM_FIBER_DETACH: &str = "fiber_detach";
/// Legacy runtime symbol for dispatch tag `FiberJoinStatus` (fiber_join_status)
pub const SYM_FIBER_JOIN: &str = "fiber_join_status";
/// Legacy runtime symbol for dispatch tag `FiberJoinValue` (fiber_join_value)
pub const SYM_FIBER_JOIN_VALUE: &str = "fiber_join_value";
/// Legacy runtime symbol for dispatch tag `FiberNowMillis` (fiber_now_millis)
pub const SYM_FIBER_NOW_MILLIS: &str = "fiber_now_millis";
/// Legacy runtime symbol for dispatch tag `FiberProcessorCount` (fiber_processor_count)
pub const SYM_FIBER_PROCESSOR_COUNT: &str = "fiber_processor_count";
/// Legacy runtime symbol for dispatch tag `FiberSpawn` (fiber_spawn)
pub const SYM_FIBER_SPAWN: &str = "fiber_spawn";
/// Legacy runtime symbol for dispatch tag `FiberSpawnWithCancelSlot` (fiber_spawn_with_cancel_slot)
pub const SYM_FIBER_SPAWN_WITH_CANCEL_SLOT: &str = "fiber_spawn_with_cancel_slot";
pub const SYM_FIBER_YIELD: &str = "fiber_yield";
/// Legacy runtime symbol for dispatch tag `FsDelete` (fs_delete)
pub const SYM_FS_DELETE: &str = "fs_delete";
/// Legacy runtime symbol for dispatch tag `FsExists` (fs_exists)
pub const SYM_FS_EXISTS: &str = "fs_exists";
/// Legacy runtime symbol for dispatch tag `FsMkdir` (fs_mkdir)
pub const SYM_FS_MKDIR: &str = "fs_mkdir";
/// Legacy runtime symbol for dispatch tag `FsReadText` (fs_read_text)
pub const SYM_FS_READ_TEXT: &str = "fs_read_text";
/// Legacy runtime symbol for dispatch tag `FsWriteText` (fs_write_text)
pub const SYM_FS_WRITE_TEXT: &str = "fs_write_text";
/// Legacy runtime symbol for dispatch tag `GcBytesAllocated` (gc_bytes_allocated)
pub const SYM_GC_BYTES_ALLOCATED: &str = "gc_bytes_allocated";
/// Legacy runtime symbol for dispatch tag `GcCollect` (gc_collect)
pub const SYM_GC_COLLECT: &str = "gc_collect";
/// Legacy runtime symbol for dispatch tag `GcCollectIfNeeded` (gc_collect_if_needed)
pub const SYM_GC_COLLECT_IF_NEEDED: &str = "gc_collect_if_needed";
/// Legacy runtime symbol for dispatch tag `GcExternalRootCount` (gc_external_root_count)
pub const SYM_GC_EXTERNAL_ROOT_COUNT: &str = "gc_external_root_count";
/// Legacy runtime symbol for dispatch tag `GcObjectCount` (gc_object_count)
pub const SYM_GC_OBJECT_COUNT: &str = "gc_object_count";
/// Legacy runtime symbol for dispatch tag `GcPhase` (gc_phase)
pub const SYM_GC_PHASE: &str = "gc_phase";
pub const SYM_GC_REGISTER_ROOT: &str = "gc_register_root";
pub const SYM_GC_ROOT_HANDLE: &str = "gc_root_handle";
pub const SYM_GC_UNREGISTER_ROOT: &str = "gc_unregister_root";
pub const SYM_GC_UNROOT_HANDLE: &str = "gc_unroot_handle";
pub const SYM_GC_WRITE_BARRIER: &str = "gc_write_barrier";
/// Legacy runtime symbol for dispatch tag `HubCreate` (hub_create)
pub const SYM_HUB_CREATE: &str = "hub_create";
/// Legacy runtime symbol for dispatch tag `HubRegister` (hub_register)
pub const SYM_HUB_REGISTER: &str = "hub_register";
/// Legacy runtime symbol for dispatch tag `HubUnregister` (hub_unregister)
pub const SYM_HUB_UNREGISTER: &str = "hub_unregister";
/// Legacy runtime symbol for dispatch tag `HubWaitReceiveIndex` (hub_wait_receive_index)
pub const SYM_HUB_WAIT_RECEIVE_INDEX: &str = "hub_wait_receive_index";
/// Legacy runtime symbol for dispatch tag `HubWaitReceiveStatus` (hub_wait_receive_status)
pub const SYM_HUB_WAIT_RECEIVE: &str = "hub_wait_receive_status";
/// Legacy runtime symbol for dispatch tag `HubWaitReceiveValue` (hub_wait_receive_value)
pub const SYM_HUB_WAIT_RECEIVE_VALUE: &str = "hub_wait_receive_value";
pub const SYM_INTEROP_DISPATCH_PTR: &str = "interop_dispatch_ptr";
pub const SYM_INTEROP_DISPATCH_UNIT: &str = "interop_dispatch_unit";
pub const SYM_INTEROP_DISPATCH_USIZE: &str = "interop_dispatch_usize";
/// Legacy runtime symbol for dispatch tag `MutexCreate` (mutex_create)
pub const SYM_MUTEX_CREATE: &str = "mutex_create";
/// Legacy runtime symbol for dispatch tag `MutexLock` (mutex_lock)
pub const SYM_MUTEX_LOCK: &str = "mutex_lock";
/// Legacy runtime symbol for dispatch tag `MutexTryLock` (mutex_try_lock)
pub const SYM_MUTEX_TRY_LOCK: &str = "mutex_try_lock";
/// Legacy runtime symbol for dispatch tag `MutexUnlock` (mutex_unlock)
pub const SYM_MUTEX_UNLOCK: &str = "mutex_unlock";
pub const SYM_PANIC: &str = "panic";
pub const SYM_PANIC_STR: &str = "panic_str";
/// Legacy runtime symbol for dispatch tag `ProcessExit` (process_exit)
pub const SYM_PROCESS_EXIT: &str = "process_exit";
/// Legacy runtime symbol for dispatch tag `ProcessGetpid` (process_getpid)
pub const SYM_PROCESS_GETPID: &str = "process_getpid";
pub const SYM_RUNTIME_PREEMPT_CHECK: &str = "runtime_preempt_check";
/// Legacy runtime symbol for dispatch tag `StrConcat` (str_concat)
pub const SYM_STR_CONCAT: &str = "str_concat";
/// Legacy runtime symbol for dispatch tag `StrEq` (str_eq)
pub const SYM_STR_EQ: &str = "str_eq";
/// Legacy runtime symbol for dispatch tag `StrFromBytesUtf8` (str_from_bytes_utf8)
pub const SYM_STR_FROM_BYTES_UTF8: &str = "str_from_bytes_utf8";
/// Legacy runtime symbol for dispatch tag `StrFromI64` (str_from_i64)
pub const SYM_STR_FROM_I64: &str = "str_from_i64";
/// Legacy runtime symbol for dispatch tag `StrLen` (str_len)
pub const SYM_STR_LEN: &str = "str_len";
/// Legacy runtime symbol for dispatch tag `StrNew` (str_new)
pub const SYM_STR_NEW: &str = "str_new";
/// Legacy runtime symbol for dispatch tag `StrSlice` (str_slice)
pub const SYM_STR_SLICE: &str = "str_slice";
/// Legacy runtime symbol for dispatch tag `SyscallRead` (syscall_read)
pub const SYM_SYSCALL_READ: &str = "syscall_read";
/// Legacy runtime symbol for dispatch tag `SyscallReadBytes` (syscall_read_bytes)
pub const SYM_SYSCALL_READ_BYTES: &str = "syscall_read_bytes";
/// Legacy runtime symbol for dispatch tag `SyscallWrite` (syscall_write)
pub const SYM_SYSCALL_WRITE: &str = "syscall_write";
/// Legacy runtime symbol for dispatch tag `SyscallWriteBytes` (syscall_write_bytes)
pub const SYM_SYSCALL_WRITE_BYTES: &str = "syscall_write_bytes";
/// Legacy runtime symbol for dispatch tag `TestBytesLen` (test_bytes_len)
pub const SYM_TEST_BYTES_LEN: &str = "test_bytes_len";
/// Legacy runtime symbol for dispatch tag `TestBytesPtr` (test_bytes_ptr)
pub const SYM_TEST_BYTES_PTR: &str = "test_bytes_ptr";
/// Legacy runtime symbol for dispatch tag `TtyWinsize` (tty_winsize)
pub const SYM_TTY_WINSIZE: &str = "tty_winsize";
/// Legacy runtime symbol for dispatch tag `WaitGroupAdd` (wait_group_add)
pub const SYM_WAIT_GROUP_ADD: &str = "wait_group_add";
/// Legacy runtime symbol for dispatch tag `WaitGroupCreate` (wait_group_create)
pub const SYM_WAIT_GROUP_CREATE: &str = "wait_group_create";
/// Legacy runtime symbol for dispatch tag `WaitGroupDone` (wait_group_done)
pub const SYM_WAIT_GROUP_DONE: &str = "wait_group_done";
/// Legacy runtime symbol for dispatch tag `WaitGroupWait` (wait_group_wait)
pub const SYM_WAIT_GROUP_WAIT: &str = "wait_group_wait";

/// User-facing FFI layout band for callback registration tables (independent of
/// [`crate::BESKID_RUNTIME_ABI_VERSION`]).
pub const BESKID_USER_FFI_LAYOUT_BAND: u32 = 1;

/// Kernel exports registered by the JIT / AOT linker for ABI v4.
pub const RUNTIME_EXPORT_SYMBOLS: &[&str] = &[
    SYM_ALLOC,
    SYM_BESKID_REGISTER_CALLBACKS,
    SYM_BESKID_REGISTER_HANDLERS,
    SYM_ABI_VERSION,
    SYM_COMPOSITION_BIND_PLURAL,
    SYM_COMPOSITION_CONTAINER_CREATE,
    SYM_COMPOSITION_CONTAINER_DROP,
    SYM_COMPOSITION_LAUNCH,
    SYM_COMPOSITION_REGISTER,
    SYM_COMPOSITION_RESOLVE,
    SYM_COMPOSITION_RESOLVE_PLURAL,
    SYM_COMPOSITION_SCOPE_DEPTH,
    SYM_COMPOSITION_SCOPE_ENTER,
    SYM_COMPOSITION_SCOPE_LEAVE,
    SYM_COMPOSITION_SHUTDOWN,
    SYM_DYNAMIC_CAST_CHECKED,
    SYM_DYNAMIC_CELL_CREATE,
    SYM_DYNAMIC_CELL_WRAP,
    SYM_DYNAMIC_MAP_AOT,
    SYM_DYNAMIC_MAP_FALLBACK,
    SYM_DYNAMIC_OBJECT_ALLOC,
    SYM_FIBER_YIELD,
    SYM_GC_REGISTER_ROOT,
    SYM_GC_ROOT_HANDLE,
    SYM_GC_UNREGISTER_ROOT,
    SYM_GC_UNROOT_HANDLE,
    SYM_GC_WRITE_BARRIER,
    SYM_INTEROP_DISPATCH_PTR,
    SYM_INTEROP_DISPATCH_UNIT,
    SYM_INTEROP_DISPATCH_USIZE,
    SYM_PANIC,
    SYM_PANIC_STR,
    SYM_RUNTIME_PREEMPT_CHECK,
];
