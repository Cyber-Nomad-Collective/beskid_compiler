...
impl UploadProgress {
    pub fn new(total: u64) -> Self {
        Self { total, done: Arc::new(AtomicU64::new(0)), last_draw: Arc::new(std::sync::Mutex::new(Instant::now())) }
    }
    ...
    where
        R: AsyncRead + Send,
    {
        ProgressReader { inner, progress: self.clone() }
    }
    ...
    fn note_bytes(&self, n: u64) {
        ...
        *last = Instant::now();
        let pct = done.saturating_mul(100).checked_div(self.total).unwrap_or(100).min(100);
        let bar_width = 30usize;
        let filled = ((pct as usize).saturating_mul(bar_width)) / 100;
        let bar: String = "#".repeat(filled) + &"-".repeat(bar_width.saturating_sub(filled));
        let _ = write!(io::stderr(), "\ruploading [{bar}] {done}/{total} ({pct}%)", total = self.total);
