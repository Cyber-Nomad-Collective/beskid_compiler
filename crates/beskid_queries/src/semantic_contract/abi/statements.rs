...
fn inferred_local_storage_type(
    ...
    let initializer = direct_expression_key(program, index, key, &binding.value)?;
    value_abi_type(db, initializer)?.ok_or_else(|| SemanticError::unavailable("value_abi_type"))
}
