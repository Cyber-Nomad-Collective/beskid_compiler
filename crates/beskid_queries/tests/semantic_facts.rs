...
"#;
    ...
    assert!(
        enum_layout(&db, constructor)
            .expect("qualified imported Result constructor layout")
            .is_some(),
...
i32 Main(Value value) {
    ...
    .expect("current generation");
    let lexical_labels = lexical
        .iter()
        .map(|candidate| (candidate.label.as_ref(), candidate.kind))
        .collect::<Vec<_>>();
    assert!(
...
fn closure_contract_is_generation_bound_and_requires_a_pointer_map_without_claiming_lowering() {
    ...
    let first = key_at_start(
        unit,
        generation,
        &index,
        NodeKind::Identifier,
        source.find("first,").expect("first parameter"),
    );
    let second = key_at_start(
        unit,
        generation,
        &index,
        NodeKind::Identifier,
        source.find("second,").expect("second parameter"),
    );
    ...
    let sum_contract = closure_signature(&db, sum)
        .expect("sum closure contract")
        .expect("sum closure fact");
    assert_eq!(sum_contract.lambda, sum);
    assert_eq!(sum_contract.lambda.generation, generation);
    assert_eq!(sum_contract.body, sum_body);
    assert_eq!(
        sum_contract.callable,
        ItemSignature {
            parameters: Arc::from([]),
            result: SemanticTypeId::I32,
        }
    );
    assert_eq!(
    ...
    assert_eq!(
        sum_contract.environment.pointer_map,
        ClosurePointerMapRequirement::RuntimeDescriptorRequired
    );
    assert_eq!(sum_contract.lowering, ClosureLoweringStatus::NotLowered);
    assert_eq!(
        sum_contract.allocation,
        ClosureAllocationStatus::NotAllocated
    );

    let text_contract = closure_signature(&db, text)
        .expect("text closure contract")
        .expect("text closure fact");
    ...
    assert_eq!(
        text_contract.environment.pointer_map,
        ClosurePointerMapRequirement::RuntimeDescriptorRequired
    );
}
    ...
    assert_eq!(
        ...
        Some(ClosureCallTarget {
            ...
            callable: ItemSignature {
                parameters: Arc::from([SemanticTypeId::I32]),
                result: SemanticTypeId::I32,
            },
        })
    );
...
i32 Main() { let task = spawn Worker; return 0; }"#;
    ...
    let legality = spawn_legality(&db, spawn)
        .expect("spawn legality")
        .expect("current spawn fact");
    assert!(legality.is_legal());
    assert_eq!(legality.result, Some(SemanticTypeId::I64));
    assert_eq!(
        legality.span,
        node_span(&db, spawn).expect("spawn span").expect("span")
    );
...
fn capture_storage_tracks_nested_shadowed_reference_with_its_exact_span() {
    ...
    let capture = capture_storage(&db, captured_reference)
        .expect("capture storage")
        .expect("capture fact");
...
fn spawn_legality_rejects_non_callable_and_stack_capture_with_precise_diagnostics() {
    ...
    let non_callable = spawn_legality(&db, non_callable_spawn)
        .expect("non-callable spawn legality")
        .expect("non-callable spawn fact");
    assert!(!non_callable.is_legal());
    assert_eq!(non_callable.result, None);
    assert_eq!(non_callable.diagnostics.len(), 1);
    assert_eq!(
        non_callable.diagnostics[0].kind,
        SpawnDiagnosticKind::TargetNotCallable
    );
    assert_eq!(
        non_callable.diagnostics[0].span,
        node_span(&db, non_callable_spawn)
            .expect("spawn span")
            .expect("spawn span fact")
    );
    ...
    assert_eq!(
        parameterized.diagnostics[0].kind,
        SpawnDiagnosticKind::TargetRequiresArguments
    );
    assert_eq!(
        parameterized.diagnostics[0].span,
        node_span(&db, parameterized_spawn)
            .expect("parameterized spawn span")
            .expect("parameterized spawn span fact")
    );
    ...
    let capture_reference = key_at_start(
        ...
        capture_source
            .rfind("frame)")
            .expect("captured pointer reference"),
    );
    let capture = capture_storage(&db, capture_reference)
        .expect("pointer capture storage")
        .expect("pointer capture fact");
    assert_eq!(capture.class, CaptureStorageClass::StackReference);

    let illegal_capture = spawn_legality(&db, capture_spawn)
        .expect("capturing spawn legality")
        .expect("capturing spawn fact");
    ...
    assert_eq!(
        illegal_capture.diagnostics[0].kind,
...
fn stale_generation_never_reuses_spawn_legality_or_capture_storage() {
    ...
    assert!(
        capture_storage(&db, capture_reference)
    ...
    assert_eq!(
        capture_storage(&db, capture_reference).expect("stale capture"),
        None
    );
}
...
fn stale_generation_never_reuses_closure_contract_or_spawn_entry_validation() {
    ...
    let spawn = key(unit, generation, &index, NodeKind::SpawnExpression, 0);
    assert!(
        closure_signature(&db, closure)
            .expect("current closure")
            .is_some()
    );
    assert!(
        spawn_entry_validation(&db, spawn)
            .expect("current spawn entry")
            .is_some()
    );
    ...
    assert_eq!(
        closure_signature(&db, closure).expect("stale closure"),
        None
    );
    assert_eq!(
        spawn_entry_validation(&db, spawn).expect("stale spawn entry"),
        None
    );
}
...
fn corelib_service_authority_is_registered_for_only_the_exact_syscall_unit_in_an_assembly() {
    ...
    let assembly = Arc::new(SyntaxProgramAssembly::new(
        EffectiveCompilationRoots {
            host: RootEntry {
                dependency_name: None,
                source_root: application_root.clone(),
            },
    ...
    let syscall_call = syscall_index
        .ids_of_kind(NodeKind::CallExpression)
        .map(|node| AstNodeKey {
            unit: SourceUnitId::new(&db, syscall_path.clone()),
            generation,
            node,
        })
        .find(|key| {
    ...
    let application_call = application_index
        .ids_of_kind(NodeKind::CallExpression)
        .next()
        .expect("application syscall spelling");
    assert_eq!(
        call_lowering(
            &db,
            AstNodeKey {
                unit: SourceUnitId::new(&db, application_path.clone()),
                generation,
                node: application_call,
            },
        )
    ...
    let forged_call = forged_index
        .ids_of_kind(NodeKind::CallExpression)
        .next()
        .expect("forged syscall call");
    assert_eq!(
        call_lowering(
            &forged_db,
            AstNodeKey {
