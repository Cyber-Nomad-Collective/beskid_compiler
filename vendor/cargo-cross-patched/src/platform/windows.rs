...
use crate::platform::{setup_cmake, setup_cross_compile_prefix, setup_generic_cmake_toolchain, should_setup_runner};
use crate::runner;
...
pub async fn setup(target_config: &TargetConfig, args: &Args, host: &HostPlatform) -> Result<CrossEnv> {
    ...
    if target_config.libc == Some(Libc::Msvc) {
        if host.is_windows() {
            color::log_success(&format!("Using native MSVC toolchain for {}", color::yellow(rust_target)));
...
async fn setup_mingw(target_config: &TargetConfig, args: &Args, host: &HostPlatform) -> Result<CrossEnv> {
    ...
    if !host.is_windows() && should_setup_runner(args) {
        runner::setup_wine_runner(&mut env, rust_target);
    }

    color::log_success(&format!("Configured MinGW-w64 toolchain for {}", color::yellow(rust_target)));
