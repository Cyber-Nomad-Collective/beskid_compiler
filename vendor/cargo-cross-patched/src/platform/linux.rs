...
pub async fn setup(target_config: &TargetConfig, args: &Args, host: &HostPlatform) -> Result<CrossEnv> {
    ...
    let cross_compiler_name = get_linux_folder_name(arch, libc, abi, &args.glibc_version, DEFAULT_GLIBC_VERSION);
    ...
    if should_setup_runner(args) {
        if host.is_darwin() {
            runner::setup_docker_qemu_runner(&mut env, arch, &bin_prefix, &compiler_dir, libc.as_str(), args, host)
                .await?;
        } else if host.is_linux() {
            runner::setup_qemu_runner(&mut env, arch, &bin_prefix, &compiler_dir, args, host).await?;
        }
    }
