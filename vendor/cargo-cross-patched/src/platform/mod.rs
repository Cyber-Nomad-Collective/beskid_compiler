...
pub fn has_preconfigured_cmake_toolchain(env: &HashMap<String, String>, target: &str) -> bool {
    ...
    let vars =
        [target_hyphen.as_str(), target_underscore.as_str(), "TARGET_CMAKE_TOOLCHAIN_FILE", "CMAKE_TOOLCHAIN_FILE"];
...
fn write_generic_cmake_toolchain_file(
    ...
    fs::create_dir_all(&output_dir).map_err(|source| CrossError::IoError {
        message: format!("Failed to create CMake toolchain directory at {}", output_dir.display()),
        source,
    })?;
...
pub fn render_cmake_toolchain_file(target_config: &TargetConfig, cross_env: &CrossEnv) -> String {
    let mut lines = vec![format!("# Auto-generated by cargo-cross for {}", target_config.target)];
    ...
    let c_compiler = cross_env.cc.as_deref().map(|tool| resolve_tool_path(tool, &cross_env.path));
    let cxx_compiler = cross_env.cxx.as_deref().map(|tool| resolve_tool_path(tool, &cross_env.path));
    let ar = cross_env.ar.as_deref().map(|tool| resolve_tool_path(tool, &cross_env.path));
    let linker = cross_env.linker.as_deref().map(|tool| resolve_tool_path(tool, &cross_env.path));
    ...
    if let Some(root) = cross_env.sysroot.as_deref().or(cross_env.sdkroot.as_deref()) {
...
fn resolve_tool_path(tool: &str, extra_path: &[PathBuf]) -> PathBuf {
    ...
    let path_dirs =
        std::env::var_os("PATH").map(|paths| std::env::split_paths(&paths).collect::<Vec<_>>()).unwrap_or_default();
...
pub async fn setup_cross_env(target_config: &TargetConfig, args: &Args, host: &HostPlatform) -> Result<CrossEnv> {
...
pub fn setup_cmake(env: &mut CrossEnv, cmake_generator: Option<&str>, is_windows: bool) {
    ...
    if let Some(g) = cmake_generator {
        env.extra_env.insert("CMAKE_GENERATOR".to_string(), g.to_string());
        return;
    }
    ...
    env.extra_env.insert("CMAKE_GENERATOR".to_string(), generator.to_string());
}
...
pub fn setup_cross_compile_prefix(env: &mut CrossEnv, bin_prefix: &str) {
    ...
    env.extra_env.insert("CROSS_COMPILE".to_string(), format!("{bin_prefix}-"));
}
...
async fn try_xcrun_sdk(sdk_name: &str) -> Option<PathBuf> {
    let output = Command::new("xcrun").args(["--sdk", sdk_name, "--show-sdk-path"]).output().await.ok()?;
...
fn search_xcode_apps_for_sdk(platform_name: &str, version: &str) -> Option<PathBuf> {
    ...
    for entry in entries.filter_map(std::result::Result::ok) {
        ...
        if name_str.starts_with("Xcode") && name_str.ends_with(".app") {
            let sdk_path = entry
                .path()
                .join(format!("Contents/Developer/Platforms/{platform_name}.platform/Developer/SDKs"))
                .join(format!("{platform_name}{version}.sdk"));
            if sdk_path.exists() {
                return Some(sdk_path);
            }
...
fn glob_matches(pattern: &str, filename: &str) -> bool {
    globset::Glob::new(pattern).map(|g| g.compile_matcher().is_match(filename)).unwrap_or(false)
}
...
mod tests {
    ...
    fn test_glob_matches_clang_exact() {
        ...
        assert!(glob_matches("x86_64-apple-darwin*-clang", "x86_64-apple-darwin25.2-clang"));
    }
    ...
    fn test_glob_does_not_match_clang_with_libc_suffix() {
        ...
        assert!(!glob_matches("x86_64-apple-darwin*-clang", "x86_64-apple-darwin25.2-clang++-libc++"));
    }
    ...
    fn test_glob_matches_aarch64_darwin_clang() {
        assert!(glob_matches("aarch64-apple-darwin*-clang", "aarch64-apple-darwin25.2-clang"));
        assert!(!glob_matches("aarch64-apple-darwin*-clang", "aarch64-apple-darwin25.2-clang++"));
    }
    ...
    fn test_glob_matches_ios_compiler() {
        ...
        assert!(glob_matches("arm64-apple-darwin*-clang", "arm64-apple-darwin11-clang"));
        assert!(!glob_matches("arm64-apple-darwin*-clang", "arm64-apple-darwin11-clang++"));
    }
