...
pub fn handle_keyboard(app: &mut CodeWidgetDemo, key: ratkit::KeyboardEvent) -> LayoutResult<CoordinatorAction> {
    if !key.is_key_down() {
    ...
    let key_event =
        CrosstermKeyEvent { code: key.key_code, modifiers: key.modifiers, kind: key.kind, state: KeyEventState::NONE };
...
fn should_quit(key: &ratkit::KeyboardEvent) -> bool {
    key.key_code == KeyCode::Char('q')
        || (key.key_code == KeyCode::Char('c') && key.modifiers.contains(crossterm::event::KeyModifiers::CONTROL))
}

/// Handles mouse events for the demo app.
pub fn handle_mouse(app: &mut CodeWidgetDemo, mouse: ratkit::MouseEvent) -> LayoutResult<CoordinatorAction> {
    ...
    let mouse_event =
        CrosstermMouseEvent { kind: mouse.kind, column: mouse.column, row: mouse.row, modifiers: mouse.modifiers };
    let code_event = app.widget().handle_mouse(mouse_event, app.code_area, &mut app.state);
