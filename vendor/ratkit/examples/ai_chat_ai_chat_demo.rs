...
fn format_output(session: &Session, messages: &[Message]) {
    let total_duration_ms = session.time.as_ref().map(|t| t.updated - t.created).unwrap_or(0);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Managed runtime ready at: http://localhost:{}", runtime.server().port());
    ...
    println!("\nFetching conversation for: {} ({})", latest_session.title, latest_session.id);
