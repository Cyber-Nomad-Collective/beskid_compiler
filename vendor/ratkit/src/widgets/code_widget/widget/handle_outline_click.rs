...
pub(crate) fn handle_outline_click(event: MouseEvent, area: Rect, state: &mut CodeState) -> Option<CodeEvent> {
    let entry = outline_entry_at_position(event.column, event.row, area, state.outline.len(), state.outline_hovered)
        .or_else(|| outline_entry_at_position(event.column, event.row, area, state.outline.len(), true))?;
