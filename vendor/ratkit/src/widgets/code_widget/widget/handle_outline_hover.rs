...
pub(crate) fn handle_outline_hover(event: MouseEvent, area: Rect, state: &mut CodeState) -> CodeEvent {
    let previous_hovered = state.outline_hovered;
    let previous_entry = state.outline_hovered_entry;
    let compact_entry =
        outline_entry_at_position(event.column, event.row, area, state.outline.len(), state.outline_hovered);
    let expanded_entry = outline_entry_at_position(event.column, event.row, area, state.outline.len(), true);
