    ...
    pub(crate) fn render_statusline(&self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        let (mode_text, mode_color) = match self.mode {
            MarkdownWidgetMode::Normal => {
                let color = self.app_theme.as_ref().map(|t| t.info).unwrap_or(Color::Rgb(97, 175, 239));
                (" NORMAL ".to_string(), color)
            }
            MarkdownWidgetMode::Drag => {
                let color = self.app_theme.as_ref().map(|t| t.warning).unwrap_or(Color::Rgb(229, 192, 123));
                (" DRAG ".to_string(), color)
            }
            MarkdownWidgetMode::Filter => {
                let color = self.app_theme.as_ref().map(|t| t.success).unwrap_or(Color::Rgb(152, 195, 121));
                let filter_text = self.filter.as_deref().unwrap_or("");
                (format!(" /{} ", filter_text), color)
            }
        };

        let file_bg = self.app_theme.as_ref().map(|t| t.background_panel).unwrap_or(Color::Rgb(58, 58, 58));
        let mode_fg = self.app_theme.as_ref().map(|t| t.background).unwrap_or(Color::Black);
        let file_fg = self.app_theme.as_ref().map(|t| t.text).unwrap_or(Color::White);

        let filename = self.source.source_path().and_then(|p| p.file_name()).and_then(|n| n.to_str());

        let source_line_count = self.source.line_count();
        let display_total = if source_line_count > 0 { source_line_count } else { self.scroll.total_lines };
        let current_line = self.scroll.current_line;
        let percentage = if display_total == 0 { 0 } else { (current_line * 100) / display_total.max(1) };
        let position_text = format!(" {}%/{} ", percentage, display_total);

        let position_bg = self.app_theme.as_ref().map(|t| t.text_muted).unwrap_or(Color::Rgb(171, 178, 191));
        let position_fg = self.app_theme.as_ref().map(|t| t.background).unwrap_or(Color::Black);
        ...
        let mut statusline = StatusLineStacked::new().start(
            Span::from(mode_text.clone()).style(Style::new().fg(mode_fg).bg(mode_color).add_modifier(Modifier::BOLD)),
        ...
        if let Some(stats) = &git_stats {
            let green =
                Style::new().fg(self.app_theme.as_ref().map(|t| t.success).unwrap_or(Color::Rgb(152, 195, 121)));
            let yellow =
                Style::new().fg(self.app_theme.as_ref().map(|t| t.warning).unwrap_or(Color::Rgb(229, 192, 123)));
            let red = Style::new().fg(self.app_theme.as_ref().map(|t| t.error).unwrap_or(Color::Rgb(224, 108, 117)));
            let dim = Style::new().fg(self.app_theme.as_ref().map(|t| t.text_muted).unwrap_or(Color::Rgb(92, 99, 112)));
