...
pub fn render_with_options(element: &MarkdownElement, width: usize, options: RenderOptions<'_>) -> Vec<Line<'static>> {
    match &element.kind {
        ElementKind::Heading { level, text, collapsed, .. } => {
            heading::render(element, *level, text, *collapsed, width, options.app_theme, options.show_heading_collapse)
        }
        ElementKind::HeadingBorder { level } => {
            vec![heading::render_border(element, *level, width, options.app_theme)]
        }
        ElementKind::CodeBlockHeader { language, blockquote_depth } => {
            vec![code_block::render_header(element, language, width, options.theme, *blockquote_depth)]
        }
        ElementKind::CodeBlockContent { content, highlighted, line_number, blockquote_depth } => {
            vec![code_block::render_content(
                element,
                content,
                highlighted.as_ref(),
                width,
                if options.show_line_numbers { Some(*line_number) } else { None },
                options.theme,
                *blockquote_depth,
            )]
        }
        ElementKind::CodeBlockBorder { kind, blockquote_depth } => {
            vec![code_block::render_border(element, kind, width, options.theme, *blockquote_depth)]
        }
        ElementKind::Paragraph(segments) => paragraph::render(element, segments, width, options.app_theme),
        ElementKind::ListItem { depth, ordered, number, content } => {
            list_item::render(element, *depth, *ordered, *number, content, width, options.app_theme)
        }
        ElementKind::Blockquote { content, depth } => {
            blockquote::render(element, content, *depth, width, options.app_theme)
        }
        ElementKind::TableRow { cells, is_header, .. } => {
        ...
        ElementKind::Frontmatter { fields, collapsed } => frontmatter::render(element, fields, *collapsed, width),
        ElementKind::FrontmatterStart { collapsed, context_id } => {
            vec![frontmatter::render_start(*collapsed, context_id.as_deref(), width)]
        }
        ElementKind::FrontmatterField { key, value } => frontmatter::render_field(key, value, width),
        ElementKind::FrontmatterEnd => {
            vec![frontmatter::render_end(width)]
        }
        ElementKind::Expandable { content_id, lines, max_lines, collapsed, total_lines: _ } => {
            expandable::render_expandable(element, content_id, lines, *max_lines, *collapsed, width, options.app_theme)
        }
        ElementKind::ExpandToggle { content_id, expanded, hidden_count } => {
            expandable::render_expand_toggle(element, content_id, *expanded, *hidden_count, width, options.app_theme)
        }
    }
}
