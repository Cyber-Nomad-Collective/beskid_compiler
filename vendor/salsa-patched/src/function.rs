    ...
    C: Configuration,
{
    ...
    fn view_caster(&self, zalsa: &Zalsa) -> &DatabaseDownCaster<C::DbView> {
        // Restored dependency memos can be visited before their generated query
        // accessor initializes this ingredient. Reuse the owning database's
        // registered typed view; never infer a cast from the erased pointer.
        self.view_caster
            .get_or_init(|| *zalsa.views().downcaster_for::<C::DbView>())
    }
}
    ...
    C: Configuration,
{
    ...
    unsafe fn maybe_changed_after(
        ...
        let db = unsafe { self.view_caster(zalsa).downcast_unchecked(db) };
